...
print TARGET "#ifndef INCLUDE_DSP_HEADER_MACRO_INNER_BRP\n";
print TARGET "	#error \"Please include " . $target_file . " via dsp_header_define_inner_brp_macro.h\"\n";
print TARGET "#endif\n\n";
#print TARGET "#include \"reg_access.h\"\n";
print TARGET "#include \"kal_public.h\"\n";
print TARGET "#include \"dsp_header_define_cuif_inner_brp.h\"\n\n";

#print TARGET "#ifdef __cplusplus\nextern \"C\" {\n#endif\n\n";
#print TARGET "#ifndef REG_BASE_C_MODULE\n";
#print TARGET "// ----------------- $module_name Bit Field Definitions -------------------\n\n";
#print TARGET "#define PACKING\n";
#print TARGET "typedef unsigned int FIELD;\n\n";
#my %block_declaration = ();
#output each register bit fields definitions
#foreach my $reg (@{$registers})
#{
#  next if (&Skip_Whole_Register($reg)); 
#
#	my $reg_name = $reg -> {"name"};
##	next if ($reg_name eq "PADDING");
#	$reg_name = AddPrefix($module_name, $reg_name, $s_prefix);
#	my $reg_info = "typedef PACKING union\n";
#	$reg_info .= "{\n";
#  $reg_info .= "    PACKING struct\n";
#  $reg_info .= "    {\n";
#  $reg_info .= DumpBitFields($reg);
#  $reg_info .= "    } Bits;\n";
#  $reg_info .= "    kal_uint32 Raw;\n";
#  $reg_info .= "} " . $reg_name . ", *P" . $reg_name . ";\n\n";
#  print TARGET $reg_info;
#  if (exists $reg->{"user-defined"}->{"Group"} and $reg->{"user-defined"}->{"Group"} !~ /^\s*$/) {
#  	my $blk_name = $reg->{"user-defined"}->{"Group"};
#  	if (! exists $block_declaration{$blk_name}) {
#  		$block_declaration{$blk_name} = "typedef struct\n";
#  		$block_declaration{$blk_name} .= "{\n";
#  	}
#  	my $count = $reg->{"count"};
#  	my $array_flag = "";
#  	my $address = Util::GetAbsoluteAddr($module_addr, $reg->{"address"}->{"address"}, $reg->{"width"});
#  	if ($count !~ /^\s*$/ and Util::String2Value($count) > 1) {
#  		$array_flag .= "[$count]";
#  		my $bytes_per_reg = $reg->{"width"} / 8;
#  		my $bytes_total = $bytes_per_reg * Util::String2Value($count);
#  		my $last_byte_offset = Util::String2Value($reg->{"address"}->{"address"}) + $bytes_total - 1;
#  		my $last_address = Util::GetAbsoluteAddr($module_addr, $last_byte_offset, $reg->{"width"});
#  		$address .= "..$last_address"
#  	}
#  	$block_declaration{$blk_name} .= sprintf("    %-31s %-20s //%s\n", $reg_name, "$reg->{\"name\"}$array_flag;", $address);
#  }
#}
#output register group definitions
#print TARGET "// ----------------- $module_name  Grouping Definitions -------------------\n";
#foreach my $key(keys %block_declaration) {
#	my $blk_name = $key;
#	$blk_name = AddPrefix2($module_name, $blk_name, $s_prefix);
#
#	print TARGET $block_declaration{$key};
#	print TARGET "}" . $blk_name . ", *P" . $blk_name . ";\n\n";
#}
#foreach my $blk (@{$blocks})
#{
#	my $blk_name = $blk -> {"name"};
#	$blk_name = AddPrefix2($module_name, $blk_name, $s_prefix);
#	my $blk_info = "typedef struct\n";
#	$blk_info .= "{\n";
#	$blk_info .= DumpBlocks($blk, $registers);
#	$blk_info .= "}" . $blk_name . ", *P" . $blk_name . ";\n\n";
#	print TARGET $blk_info;
#}
#output module definition
#print TARGET "// ----------------- $module_name Register Definition -------------------\n";
#print TARGET "typedef volatile PACKING struct\n";
#print TARGET "{\n";
...
#	my $blk_name = $key;
#	$blk_name = AddPrefix2($module_name, $blk_name, $s_prefix);
#	print TARGET sprintf("    %-31s %s;\n", $blk_name, $key);	
#}
#print TARGET "}$module_name" . "_REGS" . ", *P" . $module_name . "_REGS;\n\n";
#print TARGET "// ---------- $module_name Enum Definitions      ----------\n";
#foreach my $reg(@{$registers})
#{
#  next if (&Skip_Whole_Register($reg));
#
#	my $field_array = $reg -> {"field"};
#	my $reg_name = AddPrefix($module_name, $reg -> {"name"}, $s_prefix);
#	foreach my $field (@{$field_array})
#	{
#		if ($field -> {"name"} eq "RESERVED" or not exists $field -> {"user-defined"} -> {"C_Enum"} or $field -> {"user-defined"} -> {"C_Enum"} =~ /^\s*$/) {next;}
...
#		}
#		my $enums = $field -> {"user-defined"} -> {"C_Enum"};
#		my @enum_array = split /\n+/, $enums;
#		my $enum_info = "typedef enum\n";
#		$enum_info .= "{\n";
#		my $body = "";
#		foreach my $enum (@enum_array)
#		{
#			next if ($enum =~ /^\s*$/);
#			my @value_name = split /:/, $enum;
#			next if ($#value_name != 1);
#			my $enum_value = $value_name[0];
#			$enum_value =~ s/\s*(.*)\s*/$1/;
#			my $enum_name = $value_name[1];
#			$enum_name =~ s/\s*(.*)\s*/$1/;
#			my $tmp_str = $reg_name . "_" . $field -> {"name"} . "_" . $enum_name;
#			$tmp_str = AddBlanks($tmp_str, $width);
#			$body .= "    ${tmp_str}=	" . $enum_value . ",\n";
#		}
#		next if ($body =~ /^\s*$/);
#		$enum_info .= $body . "} ${reg_name}" . "_" . $field -> {"name"} . ";\n\n";
#		print TARGET $enum_info;
#	}
#}
# output MACRO
#print TARGET "// ---------- $module_name C Macro Definitions   ----------\n";
#print TARGET "extern P${module_name}_REGS g_${module_name}_BASE;\n\n";
#my $tmp_str = "#define ${module_name}_BASE";
#$tmp_str = AddBlanks($tmp_str, 63);
#print TARGET "${tmp_str}(g_${module_name}_BASE)\n\n";
