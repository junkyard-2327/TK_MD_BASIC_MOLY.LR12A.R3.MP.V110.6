...
/*  [Relationship mapping between  card-dependent context and ps_id/slot_id]
 *  (1) Case "DSS with RESET"
 *      => current_module and protocol_id are fixed
 *         sim_slot_id will be changed when DSS happens
 *  (2) Case "DSS without RESET"
 *      => current_module and protocol_id will be changed when DSS happens
 *         sim_slot_id is fixed
 */

/* Card-Dependent Context */
typedef struct
{
    ...
    */
    ...
    kal_uint8                   sms_simulation_bitmap;
    kal_uint8                   test_case_id; // to identify the tst_inject_string case id
#if defined (__SENSITIVE_DATA_MOSAIC__)
    kal_uint16                  mt_off_oa_from_end;
    kal_uint16                  mt_off_txt_from_end;
    kal_uint16                  mo_off_da_from_end;
    kal_uint16                  mo_off_txt_from_end;
#endif

    kal_uint16 current_module;          // MOD_SMSAL, MOD_SMSAL_2
    kal_uint8 protocol_id;              // protocol id, ps1 -> 0, ps2 -> 1
    kal_uint8 sim_slot_id;              // sim slot id, SIM1 -> 0, SIM2 -> 1
    kal_uint32                  smsal_cur_operator;     
    
} smsal_context_struct;

#ifdef __DSS_NO_RESET__
/* Protocol-Dependent Context */
typedef struct
{
    kal_uint16 current_module;          // MOD_SMSAL, MOD_SMSAL_2
    ...
    smsal_context_struct *context_ptr; // the card-dependent context that the current protocol maps
    
} smsal_ps_context_struct;
#endif
...
#ifdef __DSS_NO_RESET__
extern smsal_ps_context_struct smsal_ps_context_g[SMSAL_MAX_SIM_NUM];
extern smsal_ps_context_struct *smsal_ps_context_ptr_g;
#endif
