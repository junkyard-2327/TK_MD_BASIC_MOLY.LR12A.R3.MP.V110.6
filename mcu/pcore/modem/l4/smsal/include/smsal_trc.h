...
BEGIN_TRACE_MAP(MOD_SMSAL)
   ...
   TRC_MSG(FUNC_SMSAL_CHECK_MEM_FULL_STATUS, "SMSAL: smsal_check_mem_full_status()")
   TRC_MSG(FUNC_SMSAL_ME_RECONFIGURE_REQ_HDLR, "SMSAL: smsal_me_reconfigure_req_handler(), reconfig_type=%d")
   ...
   TRC_MSG(INFO_SMSAL_COPY_TO_BUFFER_BEFORE_RESET, "SAT_FILE_CHANGE_IND received, Need to Store COMMON PARAM before reset")
   TRC_MSG(INFO_SMSAL_COPYBACK_TO_CONTEXT_AFTER_RESET, "RESET is done , Now copy values to context from buffer")
   ...
   TRC_MSG(FUNC_SMSAL_SIM_ERROR_IND_HDLR, "SMSAL: enter smsal_sim_error_ind_hdlr")
   ...
   TRC_MSG(SMSAL_NON_SENSTIVE_DATA,"[USIR]SMSAL:Masked data is : %d, index is :%d")
   TRC_MSG(INFO_SMMA_IMS, "SMSAL[SMMA]: IMS support is enabled , send SMMA over IMS domain")
   TRC_MSG(INFO_SMMA_CS, "SMSAL[SMMA]: SMS over IMS is failed OR IMS support is disabled, send SMMA over CS  domain")
   TRC_MSG(INFO_SMSAL_SEND_SMMA, "SMSAL[SMMA]: SMMA procedure . The value of pending_smma is = %d")
   ...
   TRC_MSG(INFO_SMSAL_WAC_PRESENCE,"SMSAL_WEA_GEOFENCING: WAC is present and PDU is correct")
   TRC_MSG(INFO_SMSAL_CB_TEXT_MODE,"SMSAL_WEA_GEOFENCING: WEA is enabled , but TEXT mode is not supported")
   ...
   TRC_MSG(VM_SIM_FULL, "SMSAL: SIM Full, forward Voice mail to AP")

   TRC_MSG(INFO_SMS_IN_SIM_AND_ME,"SMSAL: sm_full=%Mkal_bool, me_full=%Mkal_bool, mem_full=%Mkal_bool")
   ...
   TRC_MSG(SMSAL_NULL_PEER_BUFF, "[USIR]SMSAL: Peer Buffer is NULL do no mask PDU")
   TRC_MSG(SMSAL_DECODE_PDU_FAIL, "[USIR]SMSAL: DATA corruption, mask all data")
   TRC_MSG(SMSAL_UPDATE_OFFSET_FAIL, "[USIR]RESET to invalid because tried to update the offset when it was Already SET!!!!")
   ...
   TRC_MSG(SMSAL_WRONG_MSG_IN_SIM_ENTRY_FUNC, "SMSAL: recv wrong msg in sim_entry_func: msgid=%d, ignore this ILM")
   TRC_MSG(SMSAL_WRONG_MSG_IN_NVM_ENTRY_FUNC, "SMSAL: recv wrong msg in nvm_entry_func: msgid=%d, ignore this ILM")
   ...
   /* for dynamic SBP/SIM switch */
   TRC_MSG(SMSAL_DYNAMIC_SBP, "SMSAL: SBP is changed, reload bearer service in NVRAM")
   TRC_MSG(SMSAL_DYNAMIC_SIM_SWITCH, "SMSAL: dynamic SIM switching") 
   TRC_MSG(SMSAL_PS_CONTEXT, "SMSAL: smsal_ps_context: ps_id %d -> slot_id %d, card_context(0x%x)")
   TRC_MSG(SMSAL_CARD_CONTEXT, "SMSAL: smsal_g(card_context): card_context(0x%x) -> ps_id %d")


   TRC_MSG(FUNC_SET_STUB, "SMSAL: smsal_set_sms_stub()")

   TRC_MSG(SMSAL_CB_TIMER_ALREADY_EXIST, "[CB_DUP_DET]: CB timer already exists with invoke_id = %d")
   TRC_MSG(SMSAL_CB_TIMER_ID_AVAILABLE, "[CB_DUP_DET]: Available timer id =  %d")
   TRC_MSG(SMSAL_CB_TIMER_ID_NOT_AVAILABLE, "[CB_DUP_DET]: NO CB timer available!!!")
   TRC_MSG(SMSAL_TIMER_NOT_NEEDED, "[CB_DUP_DET]: No need to start timer. Timer already exist!!!")
   TRC_MSG(SMSAL_FUNC_ENTRY_GS_CHANGE_IND, "[CB_DUP_DET]: smsal_cb_gs_change_ind_handler()")
   TRC_MSG(SMSAL_FUNC_ENTRY_START_DUP_DET, "[CB_DUP_DET]: smsal_cb_start_duplicate_detection_timer()")
   TRC_MSG(SMSAL_CB_TIMER_NOT_EXIST, "[CB_DUP_DET]: CB timer with given mi and sn does not exists")
   TRC_MSG(SMSAL_CB_TIMER_EXPIRY, "[CB_DUP_DET]: smsal_cb_dup_detect_timeout called for invoke_id = %d")

   TRC_MSG(SMSAL_CUR_OPERATOR, "Current operator SBP ID got from DSBP is = %d ")
