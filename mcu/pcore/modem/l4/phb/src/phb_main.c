...
/* under construction !*/
    ...
    {
        kal_uint16 cblk_num = 0;
	    kal_trace(TRACE_INFO, INFO_PHB_STARTUP_STATE, phb_ptr->state);
        for (cblk_num = 0; cblk_num < PHB_CONCURRENCY_DEGREE; cblk_num++)
        {
            if (phb_ptr->state == PHB_STATE_READY)
            {
                control_block = &phb_ptr->control_block[cblk_num];
					...
					(control_block->is_allocated == KAL_TRUE))
	            {
	                ...
	                switch (control_block->cnf_msg_id)
	                {
	                    case MSG_ID_L4CPHB_READ_REQ:
						    phb_read_error_handler(control_block);
						    break;
