...
#if defined(__CDMA2000_RAT__)
	if(phb_util_is_cdma_card() || phb_try_csim_fdn(control_block->primary_ID))
	{
	  control_block->res_msg_id = MSG_ID_UIM_FILE_INFO_CNF;
          phb_send_cdma_ilm(MOD_UIM, MSG_ID_UIM_FILE_INFO_REQ, sim_file_info_req, NULL);
	}
	else
#endif
        {
...
kal_bool phb_sim_search_record(control_block_type *control_block, data_desc_enum type)
{
    ...
    if (control_block->IO_stage == IO_NONE)
    {
		...
		sim_seek_record_req->sfi = control_block->sfi;
                #endif  
	...
	{
          control_block->res_msg_id = MSG_ID_UIM_SEARCH_RECORD_CNF;
          phb_send_cdma_ilm(MOD_UIM, MSG_ID_UIM_SEARCH_RECORD_REQ, sim_seek_record_req, NULL);
	}
	else
#endif
       {
...
void phb_read_ln_from_sim(control_block_type *control_block)
{
    ...
    if(phb_util_is_cdma_card())
    {
	phb_send_cdma_ilm(MOD_UIM, MSG_ID_UIM_READ_REQ, param_ptr, NULL);
    }
    else
#endif
...
#if defined(__CDMA2000_RAT__)
    if(phb_util_is_cdma_card() || phb_try_csim_fdn(file_idx))
    {
      phb_send_cdma_ilm(MOD_UIM, MSG_ID_UIM_READ_REQ, sim_read_req, NULL);
    }
    else
#endif
                ...
                kal_uint8 access_id)
{
    ...
    if(phb_util_is_cdma_card() || phb_try_csim_fdn(file_idx))
    {
 	phb_send_cdma_ilm(MOD_UIM, MSG_ID_UIM_WRITE_REQ, sim_write_req, NULL);
    }
    else
#endif
    ...
    switch(file_id)
    {
        ...
        case FILE_U_EMAIL_IDX:
        {
            ...
            if (*pdu_ptr != 0xFF)
            {
                ...
                email_entry->email_dcs = istring.charset;
        ...
        case FILE_U_SNE_IDX:
        {
            ...
            if ((file_id == FILE_G_SNE_IDX) || (file_id == FILE_U_SNE_IDX))
            {
                alpha_id_length = (kal_uint8)phb_desc_get_alpha_len(phb_data_desc_file_id_to_desc(file_id, 1));
            }
            else
            {
                ...
                kal_uint8 access_id)
{
    ...
    phb_desc_get_file_path(file_idx, occurrence, pbr_rec, sim_read_req->path, &(sim_read_req->sfi));
                ...
                kal_bool is_segmented)
{
    ...
    phb_desc_get_file_path(file_idx, occurrence, pbr_rec, sim_write_req->path, &(sim_write_req->sfi));
...
static void phb_IO_sim_read_cnf(ilm_struct *ilm_ptr, control_block_type *control_block)
{
...
#if defined(__CDMA2000_RAT__)
	if(phb_util_is_cdma_card() || phb_try_csim_fdn(cntx->file_id))
	{
     	   control_block->res_msg_id = MSG_ID_UIM_READ_CNF;
	}
	else
    ...
    {
    	if(cntx->IO_stage == IO_SEGMENTATION && sim_read_cnf->result == SIM_RECORD_NOT_FOUND)
        {
                phb_data_desc_mark_free(
                    phb_data_desc_file_id_to_desc(cntx->file_id, cntx->occurrence),
                    cntx->pbr_rec,
    ...
    switch (file_id)
    {
        ...
        case FILE_U_EMAIL_IDX:
        {
            ...
            if (email_entry->email_length > 0 && alpha_id_length > 0)
            {
                ...
                istr.charset = email_entry->email_dcs; //istr.charset = CHARSET_ASCII;
