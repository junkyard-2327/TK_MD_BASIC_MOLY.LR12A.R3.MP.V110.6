...
{
   sim_file_index_enum primary_ID; 
   phb_info_chk_enum is_csim;
   data_desc_enum desc;
   c2k_service_id_enum sst_id;
   sim_info_type_enum info_type;
   kal_bool (*child_req)(control_block_type *cblk);
   void (*child_cnf)(ilm_struct *ilm_ptr, control_block_type *cblk);
#if 0   
...
#if defined(__CDMA2000_RAT__)
static phb_uim_query_entry_struct phb_uim_info_tbl[] =
{
    /* ECC info */
#ifdef __PHB_USIM_SUPPORT__
    {
...
static void phb_search_iap_record_cnf(ilm_struct *ilm_ptr, control_block_type *cblk)
{
    ...
    {
        phb_desc_get_file_path(cblk->primary_ID, 1, (kal_uint8)cblk->actual_count, cblk->path, &(cblk->sfi));
...
static kal_bool phb_search_iap_record_req(control_block_type *cblk)
{
    ...
    if (phb_ptr->data_desc[DATA_DESC_PBR].record_num > 0 && phb_ptr->data_desc[DATA_DESC_IAP].is_support) // seek for usim only
    {
        ...
        if (phb_desc_get_file_path(cblk->primary_ID, 1, (kal_uint8)cblk->actual_count, cblk->path, &(cblk->sfi)))
		{
...
/* under construction !*/
    ...
    if (cblk->actual_count <= cblk->total &&
        cblk->actual_count <= phb_ptr->data_desc[DATA_DESC_PBR].record_num)
    {
        phb_desc_get_file_path(cblk->primary_ID, 1, (kal_uint8)cblk->actual_count, cblk->path, NULL);
...
static kal_bool phb_get_adn_info_req(control_block_type *cblk)
{
    ...
    if (phb_ptr->data_desc[DATA_DESC_PBR].record_num > 0)
    {
        cblk->actual_count = 1;
        cblk->total = phb_ptr->data_desc[DATA_DESC_PBR].record_num;
        if (phb_desc_get_file_path(cblk->primary_ID, 1, (kal_uint8)cblk->actual_count, cblk->path, NULL))
		{
            phb_sim_file_info(cblk, SIM_REC_SIZE);
            return KAL_TRUE;
		}
...
static void phb_get_addition_info_cnf(ilm_struct *ilm_ptr, control_block_type *cblk)
{
    ...
    {
        if (phb_desc_get_file_path(cblk->primary_ID, (kal_uint8)cblk->occurrence, cblk->actual_count, cblk->path, NULL))
        {
            phb_sim_file_info(cblk, SIM_REC_SIZE);
            return;
        }
...
static kal_bool phb_get_addition_info_req(control_block_type *cblk)
{
    ...
    switch (cblk->primary_ID)
    {
        ...
        case FILE_G_EMAIL_IDX:
			...
			desc = DATA_DESC_EMAIL;
            break;
        default:
            return KAL_FALSE;
    }
    cblk->occurrence = 1;
	if(phb_desc_is_type2_file(desc) && phb_ptr->data_desc[DATA_DESC_IAP].is_support == KAL_FALSE)
	{
		for(i = 0; i < cblk->total; i++)
		{
			phb_data_desc_set_is_support(desc + i, KAL_FALSE);
		}
		return KAL_FALSE;
	}
...
void phb_is_sim_file_support(void)
{
	...
	{
	        phb_data_desc_set_is_support(DATA_DESC_ADN, phb_UIM_service_table_Query(C2K_SERVICE_ADN));
	        phb_data_desc_set_is_support(DATA_DESC_EXT1, phb_UIM_service_table_Query(C2K_SERVICE_EXT1));

	        phb_data_desc_set_is_support(DATA_DESC_FDN, phb_UIM_service_table_Query(C2K_SERVICE_FDN));

	        phb_data_desc_set_is_support(DATA_DESC_BDN, KAL_FALSE);

	        phb_data_desc_set_is_support(DATA_DESC_MSISDN, KAL_FALSE);

	        phb_data_desc_set_is_support(DATA_DESC_SDN, phb_UIM_service_table_Query(C2K_SERVICE_SDN));

	        phb_data_desc_set_is_support(DATA_DESC_EXT2, phb_UIM_service_table_Query(C2K_SERVICE_EXT2));

	        phb_data_desc_set_is_support(DATA_DESC_EXT3, phb_UIM_service_table_Query(C2K_SERVICE_EXT3));
         }
      else
#endif
      {
...
void phb_get_info(ilm_struct *ilm_ptr, control_block_type *cblk)
{
    ...
    if(phb_util_is_cdma_card())
      {
			while (phb_query_count[curr_sim] >=0 && phb_uim_info_tbl[phb_query_count[curr_sim]].primary_ID)
			 {
				data_desc_enum desc = phb_uim_info_tbl[phb_query_count[curr_sim]].desc;
				c2k_service_id_enum sst_id;
				
				if (desc == DATA_DESC_BDN || desc == DATA_DESC_MSISDN || desc == DATA_DESC_EXT4)
				{
				...
				sst_id = phb_uim_info_tbl[phb_query_count[curr_sim]].sst_id;
				
    #ifdef __PHB_USIM_SUPPORT__
				phb_uim_info_tbl[phb_query_count[curr_sim]].primary_ID = 
					phb_desc_get_used_file_id(phb_uim_info_tbl[phb_query_count[curr_sim]].primary_ID);
    #endif
				kal_trace(TRACE_INFO, PHB_TRC_GET_CDMA_INFO, phb_uim_info_tbl[phb_query_count[curr_sim]].primary_ID);
				
				if (sst_id != C2K_SERVICE_END)
				{
				#if defined(__GEMINI__) || defined(GEMINI_PLUS) || defined(__SGLTE__)
				if (chv_status[phb_current_mod - MOD_PHB].chv1_status == CHV_BLOCKED || chv_status[phb_current_mod - MOD_PHB].chv1_status == CHV_DEAD)
				#else
				...
				{
					phb_data_desc_set_is_support(desc, phb_UIM_service_table_Query(sst_id));
				}
		...
		#ifdef __PHB_USIM_SUPPORT__
			if (phb_uim_info_tbl[phb_query_count[curr_sim]].is_csim != PHB_INFO_CHK_TOTAL)
			{
				kal_bool csim = phb_util_is_csim(curr_sim);
				if ((phb_uim_info_tbl[phb_query_count[curr_sim]].is_csim == PHB_INFO_CHK_USIM && !csim) ||
					(phb_uim_info_tbl[phb_query_count[curr_sim]].is_csim == PHB_INFO_READ_USIM && !csim) ||
					(phb_uim_info_tbl[phb_query_count[curr_sim]].is_csim == PHB_INFO_CHK_SIM && csim))
				{
					if (csim && phb_uim_info_tbl[phb_query_count[curr_sim]].primary_ID == FILE_ADN_IDX &&
		...
		#endif
			cblk->primary_ID = phb_uim_info_tbl[phb_query_count[curr_sim]].primary_ID;
			cblk->secondary_ID = PHB_NULL_VALUE;
			cblk->storage = phb_which_sim_storage();
		
			if (phb_uim_info_tbl[phb_query_count[curr_sim]].is_csim == PHB_INFO_READ_USIM ||
				phb_uim_info_tbl[phb_query_count[curr_sim]].is_csim == PHB_INFO_SEARCH_USIM)
			{
				phb_control_block_set(
					cblk,
					phb_uim_info_tbl[phb_query_count[curr_sim]].child_cnf,
					phb_get_info,
					phb_get_info_err_handler,
					startup_info_sim);
				
				if (phb_uim_info_tbl[phb_query_count[curr_sim]++].child_req(cblk))
				{
					return;
				}
				else
		...
		#ifdef __PHB_USIM_SUPPORT__
			if (phb_util_is_csim(curr_sim) && phb_uim_info_tbl[phb_query_count[curr_sim]].is_csim == PHB_INFO_CHK_USIM)
			{
				/* 
				 * according to 4.4.2.14, 
				 * if an EFPBR file contains more than one record, then they 
				 ...
				 */
			#if 1
				if (!phb_desc_get_file_path(cblk->primary_ID, 1, 1, cblk->path, NULL))
				{
					phb_query_count[curr_sim]++;
				#ifndef __LOW_COST_SUPPORT_COMMON__
					kal_trace(TRACE_INFO, PHB_TRC_GET_INFO_NOT_SUPPORT, __LINE__);
...
/* under construction !*/
			...
			phb_sim_file_info(cblk, phb_uim_info_tbl[phb_query_count[curr_sim]++].info_type);
			return;
		}
    ...
    if (ilm_ptr != NULL) // TODO: replace with STATE (startup_none --> startup_compare_digit)
    {
        ...
        if (phb_ptr->state == PHB_STATE_NOT_READY)
        {
          ...
          */
			...
			#ifdef __DSS_NO_RESET__
			cblk->mod_id = ilm_ptr->dest_mod_id - MOD_PHB; // PHB id
			#endif
...
static void phb_build_ecc(ilm_struct *ilm_ptr, control_block_type *cblk)
{
	...
	#ifdef __KOR_CUSTOMIZATION__
                if(is_ecc_custom_kor_operator_barred_num(
                    ecc->ecc, ECC_ENCODING_BCD))
                {
                     num_barred_ecc ++;
                     continue;
                }
...
static void phb_startup_build_index(ilm_struct *ilm_ptr, control_block_type *cblk)
{
    ...
    while (phb_query_count[curr_sim] >=0 && phb_build_index_tbl[phb_query_count[curr_sim]].primary_ID)
    {
...
#if defined(__CDMA2000_RAT__)
        if ((phb_build_index_tbl[phb_query_count[curr_sim]].is_usim == PHB_INFO_READ_USIM && (!usim && !csim)) ||
            (phb_build_index_tbl[phb_query_count[curr_sim]].is_usim == PHB_INFO_READ_SIM && (usim || csim)))
#else
        ...
        if (
#if defined(__CDMA2000_RAT__)
(phb_util_is_usim(curr_sim) || phb_util_is_csim(curr_sim)) && 
#else
phb_util_is_usim(curr_sim) && 
#endif
    ...
    #endif        
	#ifdef __CDMA2000_RAT__
	  if((cblk->primary_ID == FILE_U_ECC_IDX) && phb_util_is_cdma_card())
	  {
		phb_build_index_tbl[phb_query_count[curr_sim]].file_type = SIM_FILE_TRANSP;
	  }
	#endif
...
#endif
        {
            if (
               ...
               (usim || csim)
                ...
                usim
#endif
                && phb_build_index_tbl[phb_query_count[curr_sim]].primary_ID == FILE_ADN_IDX &&
                phb_ptr->data_desc[DATA_DESC_PBR].record_num == 0)
            {
                /* This is special USIM card which does not support USIM phonebook */
                /* So we read SIM phonebook and make SIM storage accessible */
 ...
 *  void
 *****************************************************************************/
void phb_startup_confirm(phb_errno_enum result, control_block_type *cblk)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
    ...
    kal_uint32 i = 0;
     ...
     kal_uint8 mod = cblk->mod_id;
	
	   #if defined(__CDMA2000_RAT__)
	    uim_phb_startup_cnf_struct *uim_phb_startup_cnf = NULL;
	    kal_bool is_cdma_card, is_uim_startup;
	  #endif
	...
	{	
      kal_trace(TRACE_INFO, INFO_PHB_DSS_STARTUP_CNF, cblk->ID, cblk->res_msg_id, mod); 
	}
    #endif
	...
	#if defined (__DSS_NO_RESET__) && defined(__CDMA2000_RAT__)
	  is_cdma_card = phb_util_is_cdma_card();	
	  is_uim_startup = ((cblk->cnf_msg_id == MSG_ID_UIM_PHB_STARTUP_REQ) ? KAL_TRUE : KAL_FALSE);

	  if(is_cdma_card && is_uim_startup)
	  {
	    uim_phb_startup_cnf = (uim_phb_startup_cnf_struct*) construct_local_para(sizeof(uim_phb_startup_cnf_struct), TD_RESET);
        uim_phb_startup_cnf->result = result;
	  }
    #endif
...
#ifdef __CDMA2000_RAT__
	|| (cblk->cnf_msg_id == MSG_ID_UIM_PHB_STARTUP_REQ)
#endif
	)
    {
        if (result == PHB_ERRNO_BUSY)
        {
            phb_send_ilm(MOD_L4C, MSG_ID_L4CPHB_STARTUP_CNF, (local_para_struct*) l4cphb_startup_cnf, NULL);
			  ...
			  if(is_cdma_card && is_uim_startup)
			  {
			    phb_send_cdma_ilm(MOD_UIM, MSG_ID_UIM_PHB_STARTUP_CNF, uim_phb_startup_cnf, NULL);
			  }
			#endif
            return;
        }
	...
	#if defined(__CDMA2000_RAT__)
		#if defined (__DSS_NO_RESET__)
		if ((result == PHB_ERRNO_SIM_SWITCH) && (cblk->cnf_msg_id == MSG_ID_UIM_PHB_STARTUP_REQ))
		{
			l4cphb_startup_cnf->result = PHB_ERRNO_SIM_SWITCH;
			if(is_cdma_card)
			{
			  uim_phb_startup_cnf->result = PHB_ERRNO_UIM_SWITCH;
			}			
		}
		#endif
	#endif
		...
		{
		  #if defined(__GEMINI__) || defined(GEMINI_PLUS) || defined(__SGLTE__)
		  kal_trace(TRACE_INFO, INFO_PHB_DSS_INFO, phb_ptr->state, phb_current_mod); 		  
		  #endif
		  msg_send6((module_type)(MOD_PHB + mod), (module_type)(MOD_L4C + mod), PHB_L4C_SAP, MSG_ID_L4CPHB_STARTUP_CNF, (local_para_struct*) l4cphb_startup_cnf, NULL);

		  #if defined(__CDMA2000_RAT__)
		  if(is_cdma_card && is_uim_startup)
		  {
		    msg_send5((module_type)(MOD_PHB + mod), (module_type)(MOD_UIM + mod), PS_CUIM_SAP, MSG_ID_UIM_PHB_STARTUP_CNF, (local_para_struct*)uim_phb_startup_cnf);
		  }
		  #endif
		}
		else
        ...
        phb_send_ilm(MOD_L4C, MSG_ID_L4CPHB_STARTUP_CNF, (local_para_struct*) l4cphb_startup_cnf, NULL);
			  ...
			  if(is_cdma_card && is_uim_startup)
			  {
			    phb_send_cdma_ilm(MOD_UIM, MSG_ID_UIM_PHB_STARTUP_CNF, (local_para_struct*) uim_phb_startup_cnf, NULL);
			  }
		  #endif
		}
        return;
    ...
    else
    {
	    ...
	    if (result == PHB_ERRNO_SIM_SWITCH)
	    {
		  l4cphb_startup_cnf->result = PHB_ERRNO_SAT_FILE_CHANGE;
		  #if defined(__GEMINI__) || defined(GEMINI_PLUS) || defined(__SGLTE__)
		  kal_trace(TRACE_INFO, INFO_PHB_DSS_INFO, phb_ptr->state, phb_current_mod);			
		   #endif
...
#if defined(__CDMA2000_RAT__)
	if(phb_util_is_cdma_card() && (cblk->cnf_msg_id == MSG_ID_UIM_UTK_FILE_CHANGE_IND))
        {
            l4cphb_startup_cnf->is_sim_refresh = 1; /* SIM refresh */
	    #if defined (__DSS_NO_RESET__)
	    if (result == PHB_ERRNO_SIM_SWITCH)
	    {
		  l4cphb_startup_cnf->result = PHB_ERRNO_UTK_FILE_CHANGE;  
		  #if defined(__GEMINI__) || defined(GEMINI_PLUS) || defined(__SGLTE__)
		  kal_trace(TRACE_INFO, INFO_PHB_DSS_INFO, phb_ptr->state, phb_current_mod);		  
		  #endif
