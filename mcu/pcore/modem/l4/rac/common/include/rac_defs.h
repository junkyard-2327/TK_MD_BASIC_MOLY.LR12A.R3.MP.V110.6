...
typedef enum
{
    ...
    RAC_NW_LOSS_OPT_TIMER_ID,

	/*This timer is used for PLMN SEARCH to avoid COPS pending too long which lead to UE assert, RAC will stop the plmn search if timer timeout*/
	RAC_PLMN_SEARCH_GUARD_TIMER_ID,
...
#define RAC_PLMN_LIST_GUARD_TIMER 60 /* 1 min ~ 60 sec */
#define RAC_PLMN_SEARCH_GUARD_TIMER 100 /* 100s */
...
#define RAC_PLMN_LIST_GUARD_TIMER 300 /* 5 min ~ 300 sec */
#define RAC_PLMN_SEARCH_GUARD_TIMER 870 /* 14min30s, to avoid UE assert, this period shall be less than AT CMD pending guard timer period(15min)*/
...
#define RAC_NW_LOSS_OPT_TIMER_PERIOD  (90 * KAL_TICKS_1_SEC)
...
} rac_provision_para_enum;


#define NVM_SKIP_LTE_SEC_PROTECTION 0x0
...
typedef enum
{
    RAC_NW_FEATURE_NOT_CHANGE = 0, //nw feature remain the same with xMM
    RAC_NW_FEATURE_NEED_CHANGE, //lower layer enter searching, NW LOSS OPT may start, ready to send nw feature
    RAC_NW_FEATURE_CHANGE,//NW LOSS OPT feature is inactive, RAC reset emc bearer service ind and update nw feature to upper layer
} rac_nw_feature_update_status_enum;
