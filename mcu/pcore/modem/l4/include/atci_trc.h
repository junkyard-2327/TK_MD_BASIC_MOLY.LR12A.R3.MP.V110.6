...
BEGIN_TRACE_MAP(MOD_ATCI)
   ...
   TRC_MSG(FUNC_L4C_NW_BAND_INFO_RIND, "l4c_nw_band_info_rind")
   TRC_MSG(FUNC_L4C_EVAL_LTE_TM9_READ_RRSP_ENTRY, "l4c_eval_lte_tm9_read_rrsp()")
   /* fmttr*/
   ...
   TRC_MSG(FUNC_REMOVE_MF_FROM_PATHID_ENTRY, "remove_mf_from_pathid()")
...
//   TRC_MSG(FUNC_RMMI_WAKEUP_TIMEOUT_HDLR,"[DUAL TALK] rmmi_wakeup_timeout_hdlr(),type:%Mrmmi_wakeup_type_enum")
   ...
   TRC_MSG(FUNC_RMMI_EXCGLA_HDLR_ENTRY, "rmmi_excgla_hdlr()")
   ...
   TRC_MSG(FUNC_RMMI_ECSCBSW_HDLR_ENTRY, "rmmi_ecscbsw_hdlr()")
   TRC_MSG(FUNC_RMMI_ECSCBCFG_HDLR_ENTRY, "rmmi_ecscbcfg_hdlr()")
   ...
   TRC_MSG(FUNC_RMMI_ESIMAPP_HDLR_ENTRY, "rmmi_esimapp_hdlr()")
   TRC_MSG(FUNC_RMMI_ESCG_HDLR_ENTRY, "rmmi_escg_hdlr()")
...
//   TRC_MSG(INFO_PULL_GPIO, "[DUAL TALK] Pull GPIO %d")
   ...
   TRC_MSG(RMMI_URC2_OUTPUT_QUEUE, "[RMMI_OUT] (head + len)> QUEUE_LENGTH. src_id:%d, RMMI_UNSOLICITED_2: %d")
   ...
   TRC_MSG(INFO_NOT_DISPLAY_MT_CALL_NOT_APPROVED, "Not display MT call when incoming call is not approved!!!")
   TRC_MSG(INFO_CLCCS_INFO, "call id:%d, dir:%Mclcc_dir_enum, cs mode:%Mclcc_mode_enum, ccstatus:%Mclccs_ccstatus_enum, mpty:%Mclcc_mpty_enum, number type:%Mclccs_number_type_enum, ton:%d, cli present:%d, cli validity:%d")
   TRC_MSG(INFO_CLCC_INFO, "call id:%d, dir:%Mclcc_dir_enum, state:%Mclcc_state_enum, mode:%Mclcc_mode_enum, mpty:%Mclcc_mpty_enum")
   TRC_MSG(INFO_ESRVCC_INFO, "USIR_DBG:call_tbl[%d]: call_id:%d, call_mode:%d, call_dir:%d  ,call_state:%d, srvcc_number_type:%d")
   ...
   TRC_MSG(INFO_UART_QUEUE, "uart_output_queue cid = %d, head = %d, length = %d")
   TRC_MSG(INFO_AGPS_DEBUGGING, "LBS output len = %d, temp = %d")
   
   /* SIM */
   TRC_MSG(INFO_L4C_SIM_USIM_LOGIC_CH_OK,"[L4C_SIM]CHANNEL OK")
   ...
   TRC_MSG(INFO_SET_AUTO_RESELECTION, "%Mplmn_selection_mode_enum to %Mplmn_selection_mode_enum, do not send AUTO_RESELECTION")
   ...
   TRC_MSG(WARNING_RMMI_SRC_CID_MAP_ERROR, "Warning: src:%hd & cid:%hd map error")
   ...
   /* SIM */
   TRC_MSG(ERROR_L4C_SIM_STATUS_ERROR,"[L4C_SIM]SIM status error")
   TRC_MSG(ERROR_L4C_SIM_USIM_LOGIC_CH_INVALID,"[L4C_SIM]CHANNEL = 0xFF")
   ...
   TRC_MSG(WARNING_INVALID_HEX_STRING, "Warning: invalid hex string")


   /* -------------------------------------------TRACE_GROUP_6, For CID and PORT------------------------------------- */
   TRC_MSG(RMMI_CID_SRC_INFO, "CID: %hd,source= %Ml4c_source_id_enum")
   ...
   TRC_MSG(FUNC_RMMI_ESIMULATE_HDLR_ENTRY, "rmmi_esimulate_hdlr()") 
