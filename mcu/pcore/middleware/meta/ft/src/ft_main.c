...
extern kal_bool Custom_META_IsAllowedLimitedStateOperation(ilm_struct* ptrMsg);
...
void FT_DispatchMessage(ilm_struct* ptrMsg)
{
...
#endif // #if !defined(__DHL_MODULE__)
    {
        ...
        if (FT_MetaSLA_Is_Enabled() && !MetaSLA_Is_Verified())
        {
            ...
            {
                char buf[128];
                sprintf(buf, "[SLA] msg id:%d is dropped since SLA is not verified", ft_header->ft_msg_id);
                dhl_print_system_trace(TRACE_INFO, buf);
                return;
            }
        }
        ...
        switch (ft_header->ft_msg_id)
        {
            ...
            {
                char buf[128];
                sprintf(buf, "[SLA] unknown msg id:%d", ft_header->ft_msg_id);
                dhl_print_system_trace(TRACE_INFO, buf);
