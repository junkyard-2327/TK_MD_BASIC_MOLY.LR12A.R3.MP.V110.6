...
AUD_CCCI_MSG_T spcIO_msgMapper(SPCIO_MSG_FROM_SPC spcIoMsg)
{
	...
	switch(spcIoMsg){
		...
		*/
		...
		case SPCIO_MSG_FROM_SPC_ENH_CTRL_SUPPORT_ACK: ccciMsgCmd = MSG_M2A_ENH_CTRL_SUPPORT_ACK; break;
...
kal_uint16 SpcIo_GetDataFromAp_StructShmBuf(const kal_uint16 offset, const kal_uint16 length, void *buf)
{
	...
	*/
	...
	if((curOffset + length) > size) {
		...
		kal_uint8 *pCurrent = buf;
		kal_mem_cpy(pCurrent, pApBuf+curOffset, sizeof(uint8)*segment);
		pCurrent += segment;
		
		segment = length - segment;
		kal_mem_cpy(pCurrent, pApBuf, sizeof(uint8)*segment);

		// move offset pointer
		curOffset = segment;
	} else {
		kal_mem_cpy(buf, pApBuf+curOffset, sizeof(uint8)*length);
...
{ 
	...
	if (gSpcIo.feature_support.support_mask == CCCI_RUNTIME_FEATURE_MUST_SUPPORT){
        ...
        if((curOffset + procLen) > (gSpcIo.shm.size - SPCIO_SHM_GUARD_PATTERN_SIZE)) {
            ...
            kal_uint8 *pCurrent = buf;
            kal_mem_cpy(pCurrent, (kal_uint8 *)(gSpcIo.shm.addr) + curOffset, sizeof(uint8)*segment);
            pCurrent += segment;

            segment = procLen - segment;
            kal_mem_cpy(pCurrent, (kal_uint8 *)(gSpcIo.shm.addr) + SPCIO_SHM_GUARD_PATTERN_SIZE, sizeof(uint8)*segment);

            // move offset pointer
            curOffset = segment + SPCIO_SHM_GUARD_PATTERN_SIZE;
        } 
        else {
            kal_mem_cpy(buf, (kal_uint8 *)(gSpcIo.shm.addr) + curOffset, sizeof(uint8)*procLen);
...
kal_int32 spcIO_copyDataToMdShareBuf_StructShmBuf(const kal_uint8 *pSrc, const kal_int16 srcLen)
{
	...
	if((curOffset + srcLen) >= bufSize) {
		...
		kal_mem_cpy( (kal_uint8 *)(pBufBase + curOffset), pSrcCur, sizeof(uint8)*segment);
		pSrcCur = pSrcCur + segment; 

		segment = srcLen - segment;
		if(segment>0) {
			kal_mem_cpy((kal_uint8 *)(pBufBase), pSrcCur, sizeof(uint8)*segment);
		}
		...
		kal_uint32 pBufBase = gSpcIo.structShmBuf->region.md_data_region.offset + gSpcIo.shm.addr;
		kal_mem_cpy((kal_uint8 *)(pBufBase + curOffset), pSrcCur, sizeof(uint8)*srcLen);
...
bool SpcIo_WriteDataToAp_viaCcciStream(void *headerBuf, kal_int16 headerLen, 
	...
	SPCIO_MSG_FROM_SPC spcIoMsg)
{
	...
	if( 0 == headerLen) {
		length = srcLen1 + srcLen2;	
	} else {
		length = sizeof(spcExtendedBufInfo) + srcLen1 + srcLen2;	
	}
	...
	if (spcIo_isMdShareBufAvailable_StructShmBuf(length)) { 
		...
		if(0 < headerLen){
			einfo.syncWord = 0x1234;
			einfo.type     = ((spcBufInfo *)headerBuf)->type;//info.type;
			einfo.length   = ((spcBufInfo *)headerBuf)->length;
