...
#define MT6306_RACE_PROTECT(a) \
    { \
        while(1) \
        { \
            kal_take_spinlock(spinlockid_mt6306_race_protect[a], KAL_INFINITE_WAIT); \
            if(!wait_sim_MT6306_RACE_PROTECT[a]) break; \
            kal_give_spinlock(spinlockid_mt6306_race_protect[a]); \
            kal_sleep_task(1); \
        } \
        wait_sim_MT6306_RACE_PROTECT[a] = KAL_TRUE; \
        Data_Sync_Barrier(); \
        kal_give_spinlock(spinlockid_mt6306_race_protect[a]); \
...
#define MT6306_RACE_RELEASE(a) \
    { \
        switch_CB->MT6306_raceConditionProtect[a] -- ; \
        wait_sim_MT6306_RACE_PROTECT[a] = KAL_FALSE; \
        Data_Sync_Barrier(); \
    }
...
#define MT6306_RACE_RELEASE(a) \
    {switch_CB->MT6306_raceConditionProtect[a] -- ;}
#endif // #if defined(DRV_SIM_6292_SERIES)
