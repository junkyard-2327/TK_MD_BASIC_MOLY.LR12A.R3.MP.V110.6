...
extern kal_uint32 SIM_GetCurrentTime(void);
    ...
    extern volatile kal_bool wait_sim_MT6306_RACE_PROTECT[];
#endif
...
#define MT6306_DBG( fmt, ...) \
    do{ \
        if(kal_if_hisr()) \
        { \
            kal_sprintf(hw_cb->hisrDbgStr, fmt,##__VA_ARGS__); \
            dhl_print(TRACE_INFO, DHL_USER_FLAG_NONE, MOD_SIM_DRV, hw_cb->hisrDbgStr); \
        } \
        else \
        { \
            kal_sprintf(hw_cb->dbgStr, fmt,##__VA_ARGS__); \
            dhl_print(TRACE_INFO, DHL_USER_FLAG_NONE, MOD_SIM_DRV, hw_cb->dbgStr); \
        } \
    }while(0)
#endif
#endif
...
//		ASSERT(0);
    ...
    {
        kal_sprintf(sim_shared_dbgstr, "[ERROR][%s] Chip:%d, Addr:%x, write:%x, readValue:%x", __func__, switch_CB->MT6306ChipNo, data >> 4, data & 0xF, readValue);
        DRV_ICC_print_str(sim_shared_dbgstr);
...
void sim_MT6306_clkStopTimeoutHandler(void *parameter)
{
    ...
    kal_take_spinlock(hw_cb->spinlockid, KAL_INFINITE_WAIT);
    hw_cb->waitGptISR_MT6306 = KAL_TRUE;
    Data_Sync_Barrier();
    kal_give_spinlock(hw_cb->spinlockid);
    if (hw_cb->needStopGptISR)
    {
        hw_cb->waitGptISR_MT6306 = KAL_FALSE;
        MT6306_DBG("\r\n[%s]: Interface:%d, Switch:%d, do not stop SIM CLK this time\r\n", __func__, hw_cb->simInterface, switch_CB->MT6306ChipNo);
        return;
    }
    ...
    if (KAL_FALSE == switch_CB->sim_MT6306_taskAccessing && KAL_FALSE == spiWriterFlag && peerHWCb->needStopGptISR == KAL_FALSE)
    {
    ...
    else if (KAL_TRUE == switch_CB->sim_MT6306_taskAccessing || peerHWCb->needStopGptISR == KAL_TRUE)
    {
...
void sim_MT6306_clkStopTimerStop(sim_HW_cb *hw_cb)
{
    ...
    {
        Data_Sync_Barrier();
        if (hw_cb->waitGptISR_MT6306 == KAL_FALSE && peerHWCb->waitGptISR_MT6306 == KAL_FALSE) break;
        kal_give_spinlock(hw_cb->spinlockid);
        kal_sprintf(hw_cb->dbgStr, "[SIM_DRV:%d][%s:%d]task waits for GPT ISR:%d, peer:%d, %x\r\n", hw_cb->simInterface, __FUNCTION__, __LINE__, (kal_uint32) hw_cb->waitGptISR_MT6306, peerHWCb->waitGptISR_MT6306, drv_get_current_time());
