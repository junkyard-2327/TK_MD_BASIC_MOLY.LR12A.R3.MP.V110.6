...
void L1sim_Configure_MT6306(kal_uint8 clockMode, sim_HW_cb *hw_cb)	//Validate
{
    ...
    kal_sprintf(hw_cb->dbgStr, "[%s], mode:%d, Interface:%d", __func__, clockMode, hw_cb->simInterface);
    DRV_ICC_print_str(hw_cb->dbgStr);
...
//kal_uint16 L1sim_Cmd(kal_uint8  *txData,kal_uint16  txSize,kal_uint8  expSize, kal_uint8  *result,kal_uint8  *rcvSize, kal_uint8 *Error)
{
    ...
    DRV_ICC_print(hw_cb, SIM_PRINT_L1SIM_CMD_TRC15, txSize, *rcvSize, index, drv_get_current_time(), SimCard->timeout);
#endif
...
///////////////////////////////////////////////////////////////////////////////////
...
#if defined(SIM_DEBUG_INFO)
        DRV_ICC_print(hw_cb, SIM_PRINT_L1SIM_CMD_TRC16, SimCard->sim_menu.addr, SimCard->sim_input.type, SimCard->sim_input.count, SW, *rcvSize);
    ...
    else
    {
        ...
        if (SW == 0x0000)
        {
            kal_sprintf(hw_cb->dbgStr, "SIM%d Get SW=0x0000 from SIM Controller!!!!",  hw_cb->simInterface);
            DRV_ICC_print_str(hw_cb->dbgStr);
        }
#endif   /*NoT0CTRL*/
...
/* under construction !*/
    ...
    {
        kal_sprintf(hw_cb->dbgStr, "%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x", result[0], result[1], result[2], result[3], result[4], result[5], result[6], result[7], result[8], result[9]);

#ifdef ATEST_DRV_ENABLE
        DRV_ICC_print_str(hw_cb->dbgStr);
#else
        kal_brief_trace(TRACE_INFO, LOG_SIM_DRV_ICCID, hw_cb->simInterface, hw_cb->dbgStr);
#endif
