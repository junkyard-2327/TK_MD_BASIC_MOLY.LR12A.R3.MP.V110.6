...
volatile kal_spinlockid spinlockid_mt6306_race_protect[SIM_MT6306_MAX_PROTECTION_NUM] = {0};
volatile kal_bool wait_sim_MT6306_RACE_PROTECT[SIM_MT6306_MAX_PROTECTION_NUM] = {KAL_FALSE};
#endif
        ...
        case (volatile usim_main_state_enum) CMD_RX_STATE:
        {
            ...
            */
            ...
            if (usim_dcb->ev_status == USIM_RX_INVALID)
            {
                ...
                kal_sprintf(hw_cb->dbgStr, "usim_dcb->ev_status Status:%x\n\r", usim_dcb->ev_status);
                DRV_ICC_print_str(hw_cb->dbgStr);
            ...
            kal_sprintf(hw_cb->dbgStr, "CMD_RX_STATE (reveive a block!! len: %d)\n\r", len);
            DRV_ICC_print_str(hw_cb->dbgStr);
#endif
...
static void usim_hisr_common(sim_HW_cb *hw_cb, kal_uint32 int_status)
{
    ...
    //kal_sprintf(hw_cb->dbgStr, "int Status:%x\n\r", int_status);
    //DRV_ICC_print_str(hw_cb->dbgStr);
    ...
    if (usim_dcb->previous_state == ACTIVATION_STATE || usim_dcb->previous_state == ATR_STATE)
    {
        ...
        if (usim_dcb->atr_count > 40)
        {
            ...
            kal_sprintf(hw_cb->dbgStr, "[SIM DRV:%d]SIM1 card send too many ATR data\n\r", hw_cb->simInterface);
            DRV_ICC_print_str(hw_cb->dbgStr);
            goto end_of_hisr;
        }
    }
...
usim_status_enum L1sim_Reset_MT6306(sim_power_enum ExpectVolt, sim_power_enum *ResultVolt, kal_bool warm, sim_HW_cb *hw_cb)
{
    ...
    {
        Data_Sync_Barrier();
        if (hw_cb->waitGptISR_MT6306 == KAL_FALSE) break;
        kal_give_spinlock(hw_cb->spinlockid);
        kal_sprintf(hw_cb->dbgStr, "[SIM_DRV:%d][%s:%d]task waits for GPT ISR:%d, %x\r\n", hw_cb->simInterface, __FUNCTION__, __LINE__, (kal_uint32) hw_cb->waitGptISR_MT6306, drv_get_current_time());
        DRV_ICC_print_str(hw_cb->dbgStr);
