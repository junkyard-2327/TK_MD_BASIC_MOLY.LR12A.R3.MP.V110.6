...
static void SIM_Initialize(kal_uint8 format, kal_uint8 power, sim_HW_cb *hw_cb)
{
    ...
    kal_sprintf(hw_cb->dbgStr, "SIM_Initialize power: %d, format: %d, TS_HSK_ENABLE: %d\n\r", power, format, SimCard->TS_HSK_ENABLE);
        ...
        while (KAL_TRUE)
        {
            ...
            if (SIM_FIFO_GetLev() == 0)
            {
                if (index > PPS_LEN)
                {
                    ...
                    kal_sprintf(hw_cb->hisrDbgStr, "Too many PTS:%d", index);
...
#if defined(ATEST_DRV_ENABLE)
    kal_sprintf(hw_cb->hisrDbgStr, "SIM_int:%x IRQEN:%x\n\r", sim_int, DRV_Reg32(SIM0_BASE_ADDR_MTK + SIM_IRQEN_MTK));
    DRV_ICC_print_str(hw_cb->hisrDbgStr);
#else
    kal_brief_trace(TRACE_INFO, LOG_SIM_DRV_HISR, hw_cb->simInterface, sim_int, DRV_Reg32(SIM0_BASE_ADDR_MTK + SIM_IRQEN_MTK));
#endif
            ...
            else
            {
                switch (SimCard->cmdState)
                {
                    ...
                    case SIM_StopClk:
                        ...
                        SimCard->t_debug[5] = ust_get_current_time_source();
...
#ifdef ATEST_DRV_ENABLE
            kal_sprintf(hw_cb->hisrDbgStr, "[SIM_DRV:%d] : null timeout %d\n\r", hw_cb->simInterface, SimCard->sim_nullByteIssueNullCount);
            DRV_ICC_print_str(hw_cb->hisrDbgStr);
#else
            kal_brief_trace(TRACE_INFO, LOG_SIM_DRV_NULL_TOUT, hw_cb->simInterface, SimCard->sim_nullByteIssueNullCount);
#endif
...
/* under construction !*/
                ...
                {
                    SIM_DisAllIntr();
                    kal_sprintf(hw_cb->hisrDbgStr, "[SIM_DRV:%d] Quit waiting null byte\n\r", hw_cb->simInterface);
                    DRV_ICC_print_str(hw_cb->hisrDbgStr);
                    SimCard->timeout = KAL_TRUE;
                    SIM_SetEvent_MTK(SimCard, SIM_NULLTIMEOUT, hw_cb);
                }
...
#ifdef ATEST_DRV_ENABLE
            kal_sprintf(hw_cb->hisrDbgStr, "[SIM_DRV:%d] : non-null timeout %d\n\r", hw_cb->simInterface, SimCard->sim_nullByteIssuenonNullCount);
            DRV_ICC_print_str(hw_cb->hisrDbgStr);
#else
            kal_brief_trace(TRACE_INFO, LOG_SIM_DRV_NON_NULL_TOUT, hw_cb->simInterface, SimCard->sim_nullByteIssuenonNullCount);
#endif
...
//kal_uint16 L1sim_Cmd(kal_uint8  *txData,kal_uint16  txSize,kal_uint8  expSize, kal_uint8  *result,kal_uint8  *rcvSize, kal_uint8 *Error)
{
    ...
    SimCard = GET_SIM_CB(hw_cb->simInterface);
    SimCard->t_debug[2] = ust_get_current_time_source();

    hw_cb->doNotStopSimClock = 0;
#ifdef SIM_DRV_EXTENDED_APDU
    if (SimCard->cmd_case == usim_case_2 || SimCard->cmd_case == usim_case_2E) // Received up to 256 in TPDU
    {
        if (expSize>256)
        {
            *rcvSize = 256;
        ...
        kal_sprintf(hw_cb->dbgStr, "[SIM_DRV:%d] Case%d, expSize:%d\r\n", hw_cb->simInterface, SimCard->cmd_case,expSize);
        	DRV_ICC_print_str(hw_cb->dbgStr);
    }
#endif
...

    {
        ...
        if (fifo_cnt != 0)
        {
            kal_sprintf(hw_cb->dbgStr, "SIM FIFO_CNT=%d after clear FIFO !!!!!!!!!!!!!", fifo_cnt);
            DRV_ICC_print_str(hw_cb->dbgStr);
        }
    }
...
/* under construction !*/
    ...
    SimCard->t_debug[3] = ust_get_current_time_source();
    ...
    if (SimCard->result == SIM_SUCCESS && SimCard->recDataErr == KAL_FALSE)
    {
        ...
        if (expSize != 0)
        {
            ...
            {
                *rcvSize = expSize;
                kal_sprintf(hw_cb->dbgStr, "SIM Error : in SIM_CMD Leftlen(%d) > expSize(%d)!!!\r\n", leftlen, expSize);
                tst_sys_trace(hw_cb->dbgStr);
            }
            else
            {
...
#if !defined(ATEST_DRV_ENABLE)
        DRV_ICC_print(hw_cb, SIM_PRINT_L1SIM_CMD_TRC16, hw_cb->simInterface/*SimCard->sim_menu.addr*/, /*SimCard->sim_input.type*/ ust_get_duration(SimCard->t_debug[2], SimCard->t_debug[3]), /*SimCard->sim_input.count*/ 0, SW, *rcvSize);
        if (result == NULL)
...
#if defined(SIM_DEBUG_INFO)
    kal_sprintf(hw_cb->dbgStr, "L1sim_Cmd(1) txSize=%d, rcvSize=%d\n\r", txSize, *rcvSize);
    DRV_ICC_print_str(hw_cb->dbgStr);
#endif
...
/* under construction !*/
        ...
        if (hw_cb->issueCardStatus == SIM_CLOCK_FETCH__TERMINAL_RESPONSE)
        {
                    ...
                    || 0x9100 == (SW & 0xFF00))
            {
                ...
                kal_sprintf(hw_cb->dbgStr, "Do not stop SIM clock this time\n\r");
                DRV_ICC_print_str(hw_cb->dbgStr);
            }
        }

#if defined(SIM_DEBUG_INFO)
        kal_sprintf(hw_cb->dbgStr, "L1sim_Cmd(2) txSize=%d, rcvSize=%d, fifo: %d\n\r", txSize, *rcvSize, SIM_Reg(SIM0_BASE_ADDR_MTK + SIM_COUNT_MTK));
        DRV_ICC_print_str(hw_cb->dbgStr);
#endif
        ...
        if (SimCard->timeout && SimCard->app_proto == USIM_PROTOCOL)
        {
            ...
            kal_sleep_task((KAL_TICKS_500_MSEC_REAL) >> 2);
#endif
            sim_PowerOff_MTK(hw_cb);
    ...
    {
        kal_sprintf(hw_cb->dbgStr, "Illegal SW:%x", SW);
        DRV_ICC_print_err_msg(hw_cb, hw_cb->dbgStr);
        sim_dump_reg(SIM_PRINT_SIM_CMD_FAIL_STATUS, hw_cb);
        *Error = KAL_TRUE;
    }
    ...
    {
        kal_sprintf(hw_cb->dbgStr, "READ BINARY, SW:%x, rcvSize:%d", SW, *rcvSize);
        DRV_ICC_print_err_msg(hw_cb, hw_cb->dbgStr);
        //*Error = KAL_TRUE;
         SW=0x6281;        
        
        kal_sprintf(hw_cb->dbgStr, "Convert SW to :%x", SW);
        DRV_ICC_print_err_msg(hw_cb, hw_cb->dbgStr);
    }
    ...
    if (SW != 0 && *Error == KAL_FALSE)
    {
        ...
        SimCard->cmd_duration_sum += ust_get_duration(SimCard->t_debug[2], SimCard->t_debug[3]);
    }
    /*SimCard->timeout's life cycle should be only in this L1sim_Cmd, reset to false before we leave*/
    SimCard->timeout = KAL_FALSE;
    /*Get ICCID*/
    ...
    {
        kal_sprintf(hw_cb->dbgStr, "%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x",
            result[0], result[1], result[2], result[3], result[4], result[5], result[6], result[7], result[8], result[9]);

#ifdef ATEST_DRV_ENABLE
        DRV_ICC_print_str(hw_cb->dbgStr);
#else
        kal_brief_trace(TRACE_GROUP_5, LOG_SIM_DRV_ICCID, hw_cb->simInterface, hw_cb->dbgStr);
#endif
...
sim_status L1sim_Cmd_Layer_MTK(kal_uint8  *txData, kal_uint32  *txSize, kal_uint8  *rxData, kal_uint32  *rxSize, sim_HW_cb *hw_cb, kal_bool *isSW6263)
{
    ...
    kal_sprintf(hw_cb->dbgStr, "[SIM_DRV:%d]%s P3=%d txSize=%d, rxData%s=NULL, *rxSize=%d\n\r",
    ...
    *isSW6263 = KAL_FALSE;
    ...
    if (rxData != NULL && *rxSize == 0)
	{
		...
		{
    		rx_buf_len = 2500;
			kal_sprintf(hw_cb->dbgStr, "[SIM_DRV:%d] Case%d, rx_buf_len:%d\r\n", hw_cb->simInterface, SimCard->cmd_case,rx_buf_len);
        	DRV_ICC_print_str(hw_cb->dbgStr);
    ...
    {
        kal_sprintf(hw_cb->dbgStr, "[SIM_DRV:%d] Case%d, tx_left_size:%d,transmitted_size:%d\r\n", hw_cb->simInterface, SimCard->cmd_case,tx_left_size,transmitted_size);
        DRV_ICC_print_str(hw_cb->dbgStr);
    ...
    else
    {
        ...
        */
        ...
        if (((txData[0] >> 4) != 0x0) && ((txData[0] >> 4) != 0x4) && ((txData[0] >> 4) != 0x6) && ((txData[0] >> 4) != 0x8))
        {
    ...
    for (;;)
    {
        ...
        {
            kal_sprintf(hw_cb->dbgStr, "[SIM_DRV:%d]return warning status1:%x \r\n", hw_cb->simInterface, status);
            DRV_ICC_print_str(hw_cb->dbgStr);
        ...
        if (sw1 == gp)
        {
            ...
            kal_sprintf(hw_cb->dbgStr, "get response  %x\r\n", sw1);
            DRV_ICC_print_str(hw_cb->dbgStr);
#endif
            if (rxData == NULL)
            {
            ...
            {
#if defined(SIM_DEBUG_INFO)
                kal_sprintf(hw_cb->dbgStr, "warning status %x %x\r\n", sw1, sw2);
                DRV_ICC_print_str(hw_cb->dbgStr);
#endif
            ...
            {
                kal_sprintf(hw_cb->dbgStr, "Rx buffer not enough, don't send GET_RESPONSE, SW2=0x%02X, rx_buff_len=%d, rx_len=%d", sw2, rx_buf_len, rx_len);
                DRV_ICC_print_str(hw_cb->dbgStr);
        ...
        else if (!sim_card && sw1 == rs)
        {
            ...
            kal_sprintf(hw_cb->dbgStr, "resend  command %x\r\n", sw1);
            DRV_ICC_print_str(hw_cb->dbgStr);
#endif
            if (rxData == NULL)
            {
                 ...
                 (sw1 == SW1_WARN1 || sw1 == SW1_WARN2 || ((status != SIM_SW_STATUS_OK) && ((sw1 & 0xf0) == 0x90) && (sw1 != 0x91) && (sw1 != 0x92))))
        {
            ...
            *isSW6263 = KAL_TRUE; //set 0x62xx 0x63xx flag
            ...
            if (sw1 == SW1_WARN2 && sw2 == 0x10)
            {
                ...
                kal_sprintf(hw_cb->dbgStr, "sw1:%x, isSW6263:%d, warn:%d, isSW6310:%d, status:%x", sw1, *isSW6263, warn, isSw6310, status);
                DRV_ICC_print_str(hw_cb->dbgStr);
                // *rxSize = rx_len; // *rxSize was used in every L1sim_Cmd, not a overall result, need to update it in the last
                // return status;
            }
            ...
            {
                *rxSize = rx_len; // *rxSize was used in every L1sim_Cmd, not a overall result, need to update it in the last
                kal_sprintf(hw_cb->dbgStr, "1.CLA:%x, INS:%x, isSW6263:%d, warn:%d, rx_len:%d, *rxSize:%x", *txData, *(txData + 1), *isSW6263, warn, rx_len, *rxSize);
                DRV_ICC_print_str(hw_cb->dbgStr);
                return 0x6310;
            }
            else
            {
                kal_sprintf(hw_cb->dbgStr, "CLA:%x, INS:%x, isSW6263:%d, warn:%d, rx_len:%d, *rxSize:%x", *txData, *(txData + 1), sw1, sw2, rx_len, *rxSize);
                DRV_ICC_print_err_msg(hw_cb, hw_cb->dbgStr);
                if (tx[1] == 0xC0 && (((*rxSize) == tx[LEN_INDEX] && tx[LEN_INDEX] != 0) || ((*rxSize) == 256 && tx[LEN_INDEX] == 0)))
                {
                    kal_sprintf(hw_cb->dbgStr, "Le(%d) == *rxSize(%d)\n\r", tx[LEN_INDEX], *rxSize);
                    DRV_ICC_print_str(hw_cb->dbgStr);
                    return  status;
                }
                else
                {
                    kal_sprintf(hw_cb->dbgStr, "Le(%d), *rxSize(%d), tx[1](%d)\n\r", tx[LEN_INDEX], *rxSize, tx[1]);
                    DRV_ICC_print_str(hw_cb->dbgStr);
                }
            }
#endif
            if (sw1 == 0x93 && sw2 == 0x00 && tx[1] == 0xC0)
            {
                kal_sprintf(hw_cb->dbgStr, "Return 9300, case4:%d, rxSize:%d,rxLen:%d", case4, *rxSize, rx_len);
                DRV_ICC_print_str(hw_cb->dbgStr);
...
#if defined(SIM_DEBUG_INFO)
            kal_sprintf(hw_cb->dbgStr, "DBG sw1:%x, isSW6263:%d, warn:%d, isSW6310:%d, status:%x", sw1, *isSW6263, warn, isSw6310, status_w);
            DRV_ICC_print_str(hw_cb->dbgStr);
#endif
            tx[LEN_INDEX] = 0;
            if (sw1 == SW1_WARN2 && sw2 == 0x10)
        ...
        else
        {
            // command complete
#if defined(SIM_DEBUG_INFO)
            kal_sprintf(hw_cb->dbgStr, "command complete %x\r\n", status);
            DRV_ICC_print_str(hw_cb->dbgStr);
#endif
            ASSERT(rx_len <= rx_buf_len);
            ...
            *rxSize = rx_len; //*rxSize was used in every L1sim_Cmd, not a overall result, need to update it in the last
            ...
            {
                kal_sprintf(hw_cb->dbgStr, "DBG2 sw1:%x, sw2:%x, isSW6263:%d, warn:%d, isSW6310:%d, status:%x", sw1, sw2, *isSW6263, warn, isSw6310, status_w);
                DRV_ICC_print_str(hw_cb->dbgStr);
...
void SIM1_LDO_enable(kal_bool enable)
{
    ...
    if (enable == KAL_TRUE)
    {
        ...
        SleepDrv_LockSleep(hw_cb->smHandler, hw_cb->sim_task_group); //lock sleep mode
#else
    ...
    else
    {
        ...
        SleepDrv_UnlockSleep(hw_cb->smHandler, hw_cb->sim_task_group);  //unlock sleep mode
#else
