 ...
 ****************************************************************************/
   ...
   TRC_MSG(SST_PHASE2_823_SIZE_MONITOR_P3, "%s: Allocate %d entries at %s line %d")
   ...
   TRC_MSG(SST_PHASE2_893_PTR_MONITOR_P1, "Task %c%c%c%c%c%c%c%c is releasing a %dB buffer pointer 0x%X from file %s")
   ...
   TRC_MSG(SST_PM_PCB, "Full content of partitioned memory control block as below:")
	...
	TRC_MSG(SST_HISTORY_FILE, "   file_name : %s")
	...
	TRC_MSG(SST_BUFFER_FOOTER_KAL2, "    KAL footer (0xF2F2 originally) = 0x%04X")
