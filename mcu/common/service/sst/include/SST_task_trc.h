 ...
 ****************************************************************************/
   ...
   TRC_MSG(SST_PHASE1_20A_TASKID_P1, "stack overflow is detected by WATCHPOINT:")
   TRC_MSG(SST_PHASE1_20A_TASKID_P2, "Stack overflow happened when PC was at 0x%08X and it tried to store to watch area 0x%08X-0x%08X")
   TRC_MSG(SST_PHASE1_20A_TASKID_P3, "Stack overflow happened when EXL or ERL bit was set or during processor was in DEBUG mode")
   TRC_MSG(SST_PHASE1_20A_TASKID_P4, "This means EPC register does not point to offending address but to the address where EXL/ERL bit was cleared")
   TRC_MSG(SST_PHASE1_20A_TASKID_P5, "Stack overflow happened when a store to watch area 0x%08X-0x%08X some point before PC reached 0x%08X")
   
   TRC_MSG(SST_PHASE1_20B_TASKID_P0, "task/hisr operation error:")
   TRC_MSG(SST_PHASE1_20B_TASKID_P1, "affinity should be resumed before set it again:")
   TRC_MSG(SST_PHASE1_20B_TASKID_P2, "resume could not be called if its never set:")
   TRC_MSG(SST_PHASE1_20B_TASKID_P3, "its not allow to set/resume affinity without configuration.")
   TRC_MSG(SST_PHASE1_20B_TASKID_P4, "affinity need to be resumed within %d minutes after setting.")
   ...
   TRC_MSG(SST_PHASE2_207_IRQID_SUB2_P1, "%c%c%c%c%c%c%c%c sufferes from stack overflow, system stack size is %dBytes.")
   ...
   TRC_MSG(SST_TC_HCB_CREATED, "tc_created : 0x%X")
	...
	/* Dump task status */
	TRC_MSG(SST_TASK_READY, "%c%c%c%c%c%c%c%c is in ready state, it is ready to execute")
	TRC_MSG(SST_TASK_READY_OSC, "%c%c%c%c%c%c%c%c is in ready state, it is ready to execute or running!")
	...
	TRC_MSG(SST_TASK_PARTITION_SUSPEND , "%c%c%c%c%c%c%c%c is in waiting for available partitoned pool!")
     TRC_MSG(SST_TASK_SCHEDULED , "%c%c%c%c%c%c%c%c is running at vpe %d at the moment")
	...
	TRC_MSG(SST_TASK_AFFINITY_SMP, "%c%c%c%c%c%c%c%c is affined to SMP, currently runnign at vpe %d")
	TRC_MSG(SST_TASK_AFFINITY, "%c%c%c%c%c%c%c%c affinity is %d (vpemask), currently runnign at vpe %d (vpemask %d)")
