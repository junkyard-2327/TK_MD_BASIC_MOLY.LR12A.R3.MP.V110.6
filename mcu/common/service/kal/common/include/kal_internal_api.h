...
#else /* __NUCLEUS_VERSION_2__ || __NUCLEUS_VERSION_3__ */
...
#define KAL_CONTEXT_IRQID_MASK 0xFFFF
#define KAL_CONTEXT_IRQID_SHIFT 16
...
#define KAL_CONTEXT_VPEID_MASK 0x7
#define KAL_CONTEXT_VPEID_SHIFT 1
#define KAL_CONTEXT_IRQ 1
/*******************************************************************************
...
#define kal_inline_is_mod_id_on_current_core(module_id)    ( (( module_id > MOD_ID_SRV_CODE_BEGIN && module_id < MOD_ID_HISR_CODE_TAIL) || module_id == MOD_TIMER) ? KAL_TRUE : KAL_FALSE )
...
*
*************************************************************************/
void *kal_get_current_context(void);
...
#if defined(__SMP_ARCH__) && defined(__MTK_TARGET__)
/*************************************************************************
* FUNCTION
...
*
*************************************************************************/
void kal_register_task_to_affinity_group(kal_taskid taskID, kal_task_group_id groupid);
...
extern void * kal_get_current_context();
...
#define KAL_ENABLE_IRQ_CALLER_CHECK(addr, vpeid)  \
    do{                                                                       \
	    if(kal_spinlock_EI_caller_address[vpeid]&&(kal_spinlock_caller_context[vpeid]==kal_get_current_context())){  \
...
#define KAL_DISABLE_IRQ_CALLER_CHECK(addr, vpeid)  \
    do{                                                                       \
	    if(kal_spinlock_DI_caller_address[vpeid]&&(kal_spinlock_caller_context[vpeid]==kal_get_current_context())){  \
...
extern void kal_clear_all_sensitive_buff_data(void);
#endif
/*************************************************************************
* FUNCTION
*  kal_debug_cbs_init
...
do { \
    ...
    register kal_uint32 _code1 __asm__ ("$4") = (kal_uint32)code1; \
    register kal_uint32 __code2 __asm__ ("$5") = _code2; \
...
#define ASSERT_EXL_SAFE(expr) do {if(!(expr)) MDM_ASSERT(0, 0, 0); } while (0)
...
typedef struct {
   task_indx_type duplicated_task_index;
   kal_char*      duplicated_task_name_core[NUM_OF_CORE];
} kal_duplicated_task_name_query_struct;
...
INLINE_MODIFIER INLINE void * kal_mem_set_init(void* dest, kal_int32 value, kal_uint32 size)
{
...
#define KAL_RT_SYSMEM_SIZE_SPINLOCK ( sizeof(kal_spinlock_type) )
#define KAL_RT_SYSMEM_SIZE_PROTECT ( sizeof(kal_protect_type) )
...
#define KAL_RT_SYSMEM_SIZE_EVSHED ( KAL_RT_SYSMEM_SIZE_KAL_TIMER + sizeof(event_scheduler_int) + (sizeof(struct TC_PROTECT_STRUCT)))
#else
#define KAL_RT_SYSMEM_SIZE_EVSHED ( KAL_RT_SYSMEM_SIZE_KAL_TIMER + sizeof(event_scheduler_int) )
#endif
#if defined(DEBUG_KAL)
...
#if defined(KAL_ON_NUCLEUS)
void kal_count_resource_usage(kal_runtime_reource_e resource, kal_bool increment);
#else
#define kal_count_resource_usage(resource, increment)
#endif
...
/* Only for pre-approved use cases. Please contact SS2 before using this interface! */
kal_status kal_hrt_take_itc_lock_nonsafe(kal_itc_lock_id itc_id, kal_int32 wait_mode);
void kal_hrt_give_itc_lock_nonsafe(kal_itc_lock_id itc_id);
