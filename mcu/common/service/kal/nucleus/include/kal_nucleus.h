...
typedef enum {
   ...
   KAL_RT_RESOURCE_SPINLOCK,
   KAL_RT_RESOURCE_PROTECT,
   KAL_RT_RESOURCE_MAX,
...
#if defined(DEBUG_DEADLOCKS)
   kal_lock_wait_info_type lock_wait_info;
#endif
   kal_uint8 task_runtime_resource_amount[KAL_RT_RESOURCE_MAX_ALIGN];
   kal_uint32 task_enhmutex_take_count;
 ...
 *************************************************************************/
typedef struct {
   CS_NODE              lock_created;
   UNSIGNED             lock_id;
   kal_uint32           lock;
   kal_uint32           interrupt_mask;
   kal_char             name[NU_MAX_NAME];
   kal_atomic_uint32    userNumber;
#if defined(DEBUG_KAL)
   void                 *owner_context;
...
#if defined(DEBUG_DEADLOCKS)
   kal_uint32           max_loop_count;
   void                 *max_loop_count_owner;
   kal_uint32           max_loop_count_taker;
   kal_uint32           max_loop_count_wait_time;
#endif
#endif
} *kal_internal_lockid, kal_lock_type;
...
#if defined(DEBUG_KAL_INTERNAL)
    kal_os_task_type    *latest_owner_task;
#endif
...
#if defined(DEBUG_KAL)
   void                 *owner_context;
   kal_uint32           spinlock_start_time;
   kal_uint32           spinlock_taker;
   kal_uint32           max_spinlock_hold_time;
   kal_uint32           max_spinlock_taker;
#if defined(DEBUG_DEADLOCKS)
   kal_uint32           max_loop_count;
   void                 *max_loop_count_owner;
   kal_uint32           max_loop_count_taker;
...
#endif
#endif
} *kal_internal_spinlockid, kal_spinlock_type;

typedef struct {
   kal_spinlock_type    protect;
} *kal_internal_protectid, kal_protect_type;
