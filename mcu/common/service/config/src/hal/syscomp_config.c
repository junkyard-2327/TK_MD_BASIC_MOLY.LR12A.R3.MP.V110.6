...
#define task_duplicated_c0c1_module_map(task, mod) task, task##_CORE0, task##_CORE1,
#define task_duplicated_c0c2_module_map(task, mod) task, task##_CORE0, task##_CORE2,
#define task_duplicated_c0c3_module_map(task, mod) task, task##_CORE0, task##_CORE3,
#define task_duplicated_c1c2_module_map(task, mod) task, task##_CORE1, task##_CORE2,
#define task_duplicated_c1c3_module_map(task, mod) task, task##_CORE1, task##_CORE3,
#define task_duplicated_c2c3_module_map(task, mod) task, task##_CORE2, task##_CORE3,
#define task_duplicated_c0c1c2_module_map(task, mod) task, task##_CORE0, task##_CORE1, task##_CORE2,
#define task_duplicated_c1c2c3_module_map(task, mod) task, task##_CORE1, task##_CORE2, task##_CORE3,
#define task_duplicated_c0c1c2c3_module_map(task, mod) task, task##_CORE0, task##_CORE1, task##_CORE2, task##_CORE3,
...
#define task_duplicated_c0c1_name(p1)       {p1,
#define task_duplicated_c0c2_name(p1)       {p1,
#define task_duplicated_c0c3_name(p1)       {p1,
#define task_duplicated_c1c2_name(p1)       {p1,
#define task_duplicated_c1c3_name(p1)       {p1,
#define task_duplicated_c2c3_name(p1)       {p1,
#define task_duplicated_c0c1c2_name(p1)     {p1,
#define task_duplicated_c1c2c3_name(p1)     {p1,
#define task_duplicated_c0c1c2c3_name(p1)   {p1,
...
#define task_affinity_group(p1)              (p1)&(~AFFINITY_DYNAMIC), ((p1)&AFFINITY_DYNAMIC ? KAL_TRUE : KAL_FALSE),
#define task_affinity_group_id(p1)           p1},

const comptask_info_struct sys_comp_config_tbl[] =
{
	#include "srv_task_config.h"
	#include "modem_2g3g_task_config.h"
	#include "modem_4g_task_config.h"
...
#define X_HISR_CONST_WITH_AFFINITY(index, hisr_index, name, priority, stack, entry, option, affinity) \
{index, priority, option, KAL_STACK_ALIGN(stack + HISR_STACK_COMMON_PLUS), entry, name, (affinity)&(~AFFINITY_DYNAMIC), ((affinity)&AFFINITY_DYNAMIC ? KAL_TRUE : KAL_FALSE)},
...
#define X_TASK_CLEAN_CONST(index, handler, option) extern void handler(void);
    #include "modem_task_clean_config.h"

#undef X_TASK_CLEAN_CONST

#define X_TASK_CLEAN_CONST(index, handler, option)  {index, handler, option, KAL_FALSE},

task_clean_s task_clean[] =
{
    #include "modem_task_clean_config.h"
};
...
const kal_uint32 total_task_clean_hdlrs = sizeof(task_clean)/sizeof(task_clean_s);
#else

const kal_uint32 total_task_clean_hdlrs = 0;
...
__attribute__ ((section ("SHOULD_NOT_USED_RODATA"))) 
#endif /* __MTK_TARGET__ */
const kal_uint32 cfg_sys_comp_config_tbl_size = sizeof( comptask_info_struct );

#ifdef __MTK_TARGET__
__attribute__ ((section ("SHOULD_NOT_USED_RODATA"))) 
#endif /* __MTK_TARGET__ */
const kal_uint32 cfg_hisr_info_size = sizeof( hisr_parameter_s );
...
#if defined(KAL_HISR_STACK_SHARING)
const kal_uint32 cfg_hisr_stack_share = 1;
#else
const kal_uint32 cfg_hisr_stack_share = 0;
#endif

#ifdef __MTK_TARGET__
__attribute__ ((section ("SHOULD_NOT_USED_RODATA"))) 
#endif /* __MTK_TARGET__ */
const kal_uint8 cfg_irq_priority_list[] =
{
