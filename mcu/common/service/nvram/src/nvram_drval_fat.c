            ...
            )
{
    ...
    if(NVRAM_IS_ATTR_RAW_DATA(attr)) {
    ...
    */
    ...
    if ((type == NVRAM_DRV_READ_TYPE_CHKSUM_ONLY)||(type == NVRAM_DRV_READ_TYPE_CHKSUM_ONLY_8B)) {
        if((attr & NVRAM_ATTR_CONFIDENTIAL) || (attr & NVRAM_ATTR_MSP)) {
            type = (type==NVRAM_DRV_READ_TYPE_CHKSUM_ONLY)?NVRAM_DRV_READ_TYPE_CHKSUM: NVRAM_DRV_READ_TYPE_CHKSUM_8B;
        }
        else {
            skip_verify = KAL_TRUE;
            working_buffer_size = section_size;
        }
    }
    ...
    if(skip_verify)
    {
        for (j = 0; j < rec_amount; j++)
        {
            ...
            */
            ...
            if(type == NVRAM_DRV_READ_TYPE_CHKSUM_ONLY){
                chksum3 = nvram_util_checksum_convert(working_buffer, NULL, KAL_TRUE);
                kal_mem_cpy(buffer + (NVRAM_CHKSUM_SIZE_2B* j), &chksum3, NVRAM_CHKSUM_SIZE_2B);
            }else{
                kal_mem_cpy(buffer + (NVRAM_CHKSUM_SIZE* j), working_buffer, NVRAM_CHKSUM_SIZE);
            }
        }
    }
    else
    {
        ...
        for (i = 0 ; i < rec_amount; )
        {
            ...
            nvram_param->Offset += (i*(section_size * rec_in_block));
            ...
            nvram_util_caculate_checksum(working_buffer + (section_size * j), rec_size, chksum1);
            ...
            chksum3 = nvram_util_caculate_checksum(working_buffer + (section_size * j), rec_size, chksum1);
            ...
            else if ((type == NVRAM_DRV_READ_TYPE_CHKSUM_8B)||(type == NVRAM_DRV_READ_TYPE_CHKSUM_ONLY_8B))
            {
            ...
            nvram_attr_enum attr)
{
    ...
    /* read RAW data directly */
    if(NVRAM_IS_ATTR_RAW_DATA(attr)) {
        if (FS_NO_ERROR > (result = FS_Read(hFile, buffer, rec_size, &len)))
        {
            nvram_fat_last_line = __LINE__;
        }
        goto ERROR;
    ...
    if(skip_verify)
    {
        for (j = 0; j < rec_amount; j++)
        {
            ...
            if(FS_NO_ERROR > (result = FS_Seek(hFile, rec_size, FS_FILE_CURRENT)))
            {
                nvram_fat_last_line = __LINE__;
                goto ERROR;
            }

            if (FS_NO_ERROR > (result = FS_Read(hFile, working_buffer, NVRAM_CHKSUM_SIZE, &len)))
            {
                nvram_fat_last_line = __LINE__;
                goto final;
            }
            if(type == NVRAM_DRV_READ_TYPE_CHKSUM_ONLY){
                chksum3 = nvram_util_cal_checksum_simple(working_buffer, NVRAM_CHKSUM_SIZE);
                kal_mem_cpy(buffer + (NVRAM_CHKSUM_SIZE_2B* j), &chksum3, NVRAM_CHKSUM_SIZE_2B);
            }else{
                kal_mem_cpy(buffer + (NVRAM_CHKSUM_SIZE* j), working_buffer, NVRAM_CHKSUM_SIZE);
            }
    ...
    nvram_attr_enum attr)
{
        ...
        buffer != NVRAM_EF_ZERO_DEFAULT && buffer != NVRAM_EF_FF_DEFAULT)
    {
        for(i = 0; i < rec_amount; i++)
        {
        	if (attr & NVRAM_ATTR_MULTI_DEFAULT)
        	{
        		...
        		nvram_util_caculate_checksum(buffer + (i+rec_index-1)*rec_size, rec_size,chksum);
			...
			else
			{
				if (i==0)
				{
					 nvram_util_caculate_checksum(buffer, rec_size,chksum);
				}
    ...
    for (i = 0; i < rec_amount; i ++)
    {
        nvram_drv_fat_prepare_data(working_buffer + rec_in_block * section_size, buffer, (i+rec_index-1) * rec_size, rec_size);
         ...
         nvram_util_caculate_checksum(working_buffer + rec_in_block * section_size, rec_size, chksum);
        ...
        buffer != NVRAM_EF_ZERO_DEFAULT && buffer != NVRAM_EF_FF_DEFAULT)
    {
        ...
        for(i = 0;i < rec_amount;i++)
        {
            ...
            if ((i == 0 || (attr & NVRAM_ATTR_MULTI_DEFAULT)) && 
                !NVRAM_IS_ATTR_RAW_DATA(attr) )
            {
                 nvram_util_caculate_checksum(buffer + i * rec_size, rec_size, chksum);
            }
            ...
            if (FS_NO_ERROR > (result = FS_Write(hFile, working_buffer, section_size, &len)))
            {
                    nvram_fat_last_line = __LINE__;
                goto final;
            }
        ...
        for (i = 0 ; i < rec_amount; i++)
        {
            for (j = 0; j < rec_size; j += working_buffer_size)
            {
                ...
                if (i == 0 || attr & NVRAM_ATTR_MULTI_DEFAULT)
                {
                    ...
                    nvram_util_caculate_checksum(working_buffer, byte_in_block, tmpchksum);
                    for(cs_idx= 0; cs_idx<8;cs_idx++){
        ...
        if (!(attr & NVRAM_ATTR_MULTI_DEFAULT))
        {
                ...
                !encrypt_data )
            {
                for(i = 0; i < rec_amount; i++) {
                    ...
                    if (FS_NO_ERROR > (result = FS_Write(hFile, (working_buffer + rec_size), NVRAM_CHKSUM_SIZE, &len))) {
                        nvram_fat_last_line = __LINE__;
                        goto final;
                    }
                }
                                          ...
                                          kal_bool initialize)
{
    ...
    do
    {
        ...
        if (!nvram_util_has_file_created(ldi))
        {
            ...
            if ((result = FS_Write(hFile, ldi_hd_buffer, NVRAM_LDI_HEADER_SIZE, &ldi_hd_buffer_size)) < FS_NO_ERROR)
            {
                nvram_fat_last_line = __LINE__;
                break;
            }
                                          ...
                                          kal_bool initialize)
{
    ...
    do
    {
        ...
        if (!(attr & NVRAM_ATTR_BACKUP_RAW) && 
            ...
            (len <= NVRAM_LDI_HEADER_SIZE)) //only have header, fill content is NULL
        {
            nvram_fat_last_line = __LINE__;
            result = NVRAM_DRV_EMPTY_RECORD;
        ...
        result = nvram_drv_fat_write_section(hFile, buffer, rec_index, rec_amount, rec_size, attr, len);
    }while(0);
                                                                ...
                                                                kal_uint32 writesize)
{
    ...
    if (Ret == offset)
    {
        ...
        if (Ret == FS_NO_ERROR)
        {   
             nvram_util_caculate_checksum(buffer, writesize,chksum);
            Ret = FS_Write(hFile, chksum, NVRAM_CHKSUM_SIZE, &len);
        }
    }
...
static kal_int32 nvram_drv_fat_read_file(const kal_wchar* filename, void *DataPtr, kal_uint32 *Readlen)
{
    ...
    do
    {
        ...
        if (FS_NO_ERROR > (result = FS_Seek(hFile, 0, FS_FILE_BEGIN)))
        {
            FS_Close(hFile);
            nvram_fat_last_line = __LINE__;
            break;
            ...
            
{
    ...
    if(is_nvram_in_ota_flow)
    {
        if(is_nvram_first_restore)
        {
            ...
            MD_FS_Delete((const kal_wchar*)Cali_filelistname);
            MD_FS_Delete((const kal_wchar*)Imei_filelistname);
