...
void nvram_log_ota_info(void)
{
    ...
    FS_Read(handle, info, sizeof(info)-1, &len);
    FS_Close(handle);
...
void nvram_write_exception(kal_uint16 length, long *ex_data, long *ex_log)
{
    ...
    */
    ...
    {
        result = nvram_read_exception_data_item(ldi, i, (kal_uint8*) & exception_header, sizeof(exception_header));
...
nvram_errno_enum nvram_validate_file(nvram_lid_enum LID, kal_wchar *path)
{
    ...
    buffer = (kal_uint8 *)get_ctrl_buffer(NVRAM_CHKSUM_SIZE_2B* ldi->total_records);
...
//    nvram_pre_init();
    ...
    * Invalid input paramters.
    */
    if (buffer_size < NVRAM_CHKSUM_SIZE_2B * rec_amount)
    {
        return KAL_FALSE;
    }
    ...
    /* handle by nvram_io layer */
    result = nvram_read_data_item_chksum(ldi, rec_index, rec_amount, (kal_uint16*)buffer, buffer_size, KAL_TRUE);
...
//    nvram_pre_init();
    ...
    */
    ...
    /* handle by nvram_io layer */
    result = nvram_read_data_item_chksum(ldi, rec_index, rec_amount, (kal_uint16*)buffer, buffer_size, KAL_FALSE);
...
//    nvram_pre_init();
    ...
    */
    ...
    /* handle by nvram_io layer */
    result = nvram_read_data_item_chksum_8b(ldi, rec_index, rec_amount, (kal_uint8*)buffer, buffer_size, KAL_TRUE);
...
//    nvram_pre_init();
    ...
    */
    ...
    /* handle by nvram_io layer */
    result = nvram_read_data_item_chksum_8b(ldi, rec_index, rec_amount, (kal_uint8*)buffer, buffer_size, KAL_FALSE);
...
kal_int32 nvram_compare_to_default_value(nvram_lid_enum LID, kal_uint16 rec_index)
{
    ...
    else
    {
        ...
        buffer = (kal_uint8*) get_ctrl_buffer(ldi->total_records * NVRAM_CHKSUM_SIZE_2B);

        /* this is temp solution, should implement full solution in drv layer in the future */
        nvram_read_data_item_chksum(ldi, 1, ldi->total_records, (kal_uint16*) buffer, ldi->total_records * NVRAM_CHKSUM_SIZE_2B, KAL_TRUE);
        ...
        for (i = (rec_index == 0?0:rec_index-1); i < ldi->total_records ; i++)
        {
            ...
            NVRAM_EXT_ASSERT(default_value != NULL,(kal_uint32)default_value, NVRAM_LOC_GET_DEFAULT_VALUE_IS_NULL_1, 0);

            chksum1 = nvram_util_caculate_checksum(default_value, ldi->size, checksum_buf);
            chksum2 = *(kal_uint16*)(buffer + NVRAM_CHKSUM_SIZE_2B * i);
...
kal_bool nvram_external_search_reserved_lid(nvram_lid_enum LID, kal_char *verno, kal_uint16 total_records, kal_uint16 size)
{
    ...
    if(size) {
        sprintf((filename + NVRAM_FILE_LEN + 5), "%04X", size);
    }
    else {
        strcat(filename, "????");
    }
    ...
    strcpy(ldi_r->fileprefix, ldi->fileprefix);
    strcpy(ldi_r->fileverno, verno);
