    ...
    usbc_io_request_t* io_request)
{
    ...
    ret = usbc_class_device_submit_io_request_priv(core_queue_hif_type, pQueue, io_request);
    HIF_SWLA_STOP("UD1");
    return ret;
}
    ...
    if ( num != 0 )
    {
        ...
        if ( pQueue->notify_complete != NULL )
        {
            if ( first_gpd == NULL )
            {
                ...
                usbc_data_trace(USBCORE_CLASS_FLUSH_SW_QUEUE, queue_no, num, (kal_uint32)first_gpd, (kal_uint32)last_gpd);
    ...
    if ( num != 0 )
    {
        ...
        if ( pQueue->notify_complete != NULL )
        {
            if ( first_gpd == NULL )
            {
                ...
                usbc_io_request_t* request = NULL;
                usbc_data_trace(USBCORE_CLASS_FLUSH_HW_QUEUE, queue_no, num, (kal_uint32)first_gpd, (kal_uint32)last_gpd);
                request = (usbc_io_request_t*)QBM_DES_GET_SW_CTRL_FIELD(first_gpd);
                request->next_request = NULL;
                request->first_gpd = first_gpd;
                request->last_gpd = last_gpd;
