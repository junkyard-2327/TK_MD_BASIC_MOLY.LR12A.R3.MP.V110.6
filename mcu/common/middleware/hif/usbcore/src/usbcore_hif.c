...
#endif
        usbc_data_trace(USBCORE_POLL_QUEUE_TX, q_no, num, (kal_uint32)first_gpd, (kal_uint32)last_gpd);
...
#ifdef __USBCORE_TARGET_DEBUG__
        usbc_data_trace(USBCORE_POLL_QUEUE_LEN_TX, q_no, gpd_len, bd_len, spd_len);
#endif
    }
}
...
void usbc_normal_hif_poll_queue_rx_gpd(kal_uint8 q_no)
{
    ...
    if(num > 0) {
        ...
        //usbc_core_dump_gpd_list(first_gpd, last_gpd);
#endif
        usbc_data_trace(USBCORE_POLL_QUEUE_RX, q_no, num, (kal_uint32)first_gpd, (kal_uint32)last_gpd);
#ifdef __USBCORE_TARGET_DEBUG__
        kal_uint32 gpd_len, bd_len, spd_len;
        usbc_core_getGpdLength(first_gpd, last_gpd, &gpd_len, &bd_len, &spd_len);
#endif
...
#ifdef __USBCORE_TARGET_DEBUG__
        usbc_data_trace(USBCORE_POLL_QUEUE_LEN_RX, q_no, gpd_len, bd_len, spd_len);
#endif
    }
}
...
void usbc_normal_hif_poll_queue_drb(kal_uint8 q_no)
{
    ...
    if(num > 0) {
        usbc_data_trace(USBCORE_POLL_QUEUE_DRB, q_no, num, startIdx, endIdx);
        ...
        if( pUsbCore->tx_queue[q_no].notify_rb_complete != NULL ) {
            ...
            pUsbCore->tx_queue[q_no].notify_rb_complete(pUsbCore->tx_queue[q_no].class_device_id, 0, startIdx, endIdx);            
        }
...
void usbc_normal_hif_poll_queue_xit(kal_uint8 q_no)
{
    usbc_core_t    *pUsbCore = usbc_core_get_instance();
    kal_uint32      num = 0;
    kal_uint16      startIdx, endIdx;
    LHIF_NET_TYPE net_type;
    kal_uint32 classId;
    ubm_xit_field_val trfLen, dataAddr;

    classId = pUsbCore->rx_queue[q_no].class_device_id;
    net_type = ubm_ul_meta_get_lhif_net_type(HIFUSB_EPNO_2_QNO(pUsbCore->rx_queue[q_no].ep_no), (pUsbCore->class_device[classId]).class_type);

    if((pUsbCore->rx_xit_meta)[q_no].xit2meta_retry == KAL_TRUE) {
        while((pUsbCore->rx_xit_meta)[q_no].remain_num > 0) {
            ubm_xit_operate(HIFUSB_EPNO_2_QNO(pUsbCore->rx_queue[q_no].ep_no), (pUsbCore->rx_xit_meta)[q_no].startIdx, UBM_XIT_OP_GET, TRFLEN, &trfLen);       // get UL transfer length
            ubm_xit_operate(HIFUSB_EPNO_2_QNO(pUsbCore->rx_queue[q_no].ep_no), (pUsbCore->rx_xit_meta)[q_no].startIdx, UBM_XIT_OP_GET, STARTADDR, &dataAddr);  // get UL transfer data address
            
            if(ubm_ul_meta_enqueue_lhif_ul_queue(HIFUSB_EPNO_2_QNO(pUsbCore->rx_queue[q_no].ep_no), net_type, classId, (void*)dataAddr.xitStartAddr, trfLen.xitXferLen)) {
                ubm_xit_release(HIFUSB_EPNO_2_QNO(pUsbCore->rx_queue[q_no].ep_no), (pUsbCore->rx_xit_meta)[q_no].startIdx, 1);

                (pUsbCore->rx_xit_meta)[q_no].startIdx = ubm_xit_get_next_idx(HIFUSB_EPNO_2_QNO(pUsbCore->rx_queue[q_no].ep_no), (pUsbCore->rx_xit_meta)[q_no].startIdx);
                (pUsbCore->rx_xit_meta)[q_no].remain_num--;
            } else {
                break;
            }
        }

        if((pUsbCore->rx_xit_meta)[q_no].remain_num > 0) {
            return;
        }
        else {
            (pUsbCore->rx_xit_meta)[q_no].xit2meta_retry = KAL_FALSE;
            (pUsbCore->rx_xit_meta)[q_no].remain_num = (pUsbCore->rx_xit_meta)[q_no].startIdx = (pUsbCore->rx_xit_meta)[q_no].endIdx = 0;
        }
    }
     ...
     * Poll XIT
     */
    num = ubm_xit_poll(HIFUSB_EPNO_2_QNO(pUsbCore->rx_queue[q_no].ep_no), &startIdx, &endIdx);

    /*
     * Enqueue to LHIF UL queue
     */
    if(num > 0) {
        usbc_data_trace(USBCORE_POLL_QUEUE_XIT, q_no, num, startIdx, endIdx);
    }

    while(num > 0) {
        ubm_xit_operate(HIFUSB_EPNO_2_QNO(pUsbCore->rx_queue[q_no].ep_no), startIdx, UBM_XIT_OP_GET, TRFLEN, &trfLen);       // get UL transfer length
        ubm_xit_operate(HIFUSB_EPNO_2_QNO(pUsbCore->rx_queue[q_no].ep_no), startIdx, UBM_XIT_OP_GET, STARTADDR, &dataAddr);  // get UL transfer data address

        if(ubm_ul_meta_enqueue_lhif_ul_queue(HIFUSB_EPNO_2_QNO(pUsbCore->rx_queue[q_no].ep_no), net_type, classId, (void*)dataAddr.xitStartAddr, trfLen.xitXferLen)) {
            ubm_xit_release(HIFUSB_EPNO_2_QNO(pUsbCore->rx_queue[q_no].ep_no), startIdx, 1);

            startIdx = ubm_xit_get_next_idx(HIFUSB_EPNO_2_QNO(pUsbCore->rx_queue[q_no].ep_no), startIdx);
            num--;
        } else {
            (pUsbCore->rx_xit_meta)[q_no].xit2meta_retry = KAL_TRUE;
            (pUsbCore->rx_xit_meta)[q_no].remain_num = num;
            (pUsbCore->rx_xit_meta)[q_no].startIdx = startIdx;
            (pUsbCore->rx_xit_meta)[q_no].endIdx = endIdx;
            break;  // LHIF UL queue is full, retry in next polling
        }
    }
}


void usbc_normal_hif_check_meta_ul()
{
    usbc_core_t    *pUsbCore = usbc_core_get_instance();
    kal_uint16 startIdx, endIdx;
    kal_uint8 i = 0;

    if(ubm_ul_meta_check_lhif_ul_queue(&startIdx, &endIdx)) {
        usbc_data_trace(USBCORE_CHK_UL_META, LHIF_HWQ_AP_UL_Q0, startIdx, endIdx);

        for ( i=0; i < pUsbCore->total_rx_queues; i++ ) {
            if ( (pUsbCore->rx_queue[i].state > USBC_CORE_QUEUE_STATE_INITIATED) && (pUsbCore->rx_queue[i].config & HIFUSB_QMU_RXQ_UL_PRB_XIT_EN) ) {
                pUsbCore->rx_queue[i].notify_rb_complete(pUsbCore->rx_queue[i].class_device_id, LHIF_HWQ_AP_UL_Q0, startIdx, endIdx);
            }
        }
    }
}

kal_bool usbc_normal_hif_meta_ul_lhif_notify(kal_uint16 startIdx, kal_uint16 endIdx, LHIF_QUEUE_TYPE qType)
{
    usbc_core_t    *pUsbCore = usbc_core_get_instance();
    kal_uint8 i = 0;

    usbc_data_trace(USBCORE_CHK_UL_META, qType, startIdx, endIdx);

    for ( i=0; i < pUsbCore->total_rx_queues; i++ ) {
        if ( (pUsbCore->rx_queue[i].state > USBC_CORE_QUEUE_STATE_INITIATED) && (pUsbCore->rx_queue[i].config & HIFUSB_QMU_RXQ_UL_PRB_XIT_EN) ) {
            pUsbCore->rx_queue[i].notify_rb_complete(pUsbCore->rx_queue[i].class_device_id, qType, startIdx, endIdx);
        }
    }
...
static kal_bool usbc_normal_hif_hw_usbq_set_gpd(hif_queue_type_e q_type, kal_uint8 queue_no, qbm_gpd* first_gpd, qbm_gpd* last_gpd)
{
    ...
    usbc_data_trace(USBCORE_CLASS_SUBMIT_IO_REQUEST_HWQ, ((q_type << 8) | (queue_no & 0xFF)), (kal_uint32)first_gpd, (kal_uint32)last_gpd);
...
static kal_bool usbc_normal_hif_sw_usbq_set_gpd(hif_queue_type_e q_type, kal_uint8 queue_no, qbm_gpd* first_gpd, qbm_gpd* last_gpd)
{
    ...
    usbc_data_trace(USBCORE_CLASS_SUBMIT_IO_REQUEST_SWQ, ((q_type << 8) | (queue_no & 0xFF)), (kal_uint32)first_gpd, (kal_uint32)last_gpd);
...
{
    ASSERT(0);      // No user in normal/dongle mode now
