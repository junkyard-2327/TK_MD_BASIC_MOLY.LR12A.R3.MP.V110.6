...
#define PFM_FIN_ACK_INVALID_FILTER_ID   -10
 ...
 * Private variables.
 *----------------------------------------------------------------------------*/
static kal_int32 g_fin_ack_filter_id_v4 = PFM_FIN_ACK_INVALID_FILTER_ID;
static kal_int32 g_fin_ack_filter_id_v6 = PFM_FIN_ACK_INVALID_FILTER_ID;
/*------------------------------------------------------------------------------
 * Private fucntions.
 *----------------------------------------------------------------------------*/
                            ...
                            kal_uint32 length)
{
    ...
    hif_data_trace(PFM_DL_RECEIVED_FILTER_PACKET, head_gpd, tail_gpd);
    ...
    /* Fill IP/TCP header */
    {
        ...
        QBM_CACHE_FLUSH(ip_header, total_len);
        QBM_DES_SET_DATALEN(bd, total_len);qbm_cal_set_checksum(bd);
        QBM_DES_SET_DATALEN(ul_gpd, total_len);qbm_cal_set_checksum(ul_gpd);
    }
...
void pfm_fin_ack_filter_register_parser(void *buf, kal_uint32 filter_cnt, kal_bool uplink)
{
    ...
    if (PFM_FIN_ACK_INVALID_FILTER_ID == g_fin_ack_filter_id_v4) {
    ...
    if (PFM_FIN_ACK_INVALID_FILTER_ID == g_fin_ack_filter_id_v6) {
...
void pfm_fin_ack_filter_deregister_callback(kal_int32 filter_id)
{
    ipc_deregister_dl_filter(g_fin_ack_filter_id_v4);
    ipc_deregister_dl_filter(g_fin_ack_filter_id_v6);
    g_fin_ack_filter_id_v4 = PFM_FIN_ACK_INVALID_FILTER_ID;
    g_fin_ack_filter_id_v6 = PFM_FIN_ACK_INVALID_FILTER_ID;
