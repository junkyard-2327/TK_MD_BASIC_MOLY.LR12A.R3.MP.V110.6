...
static void process_job(private_processor_t *this, worker_thread_t *worker)
{
	...
	if (worker->job->is_start_alarm_timer == TRUE) {
		/* Release wake lock after timerout handler executed */
		lib->scheduler->release_alarm_timer_wake_lock(lib->scheduler, worker->priority, FALSE);
		worker->job->is_start_alarm_timer = FALSE;
	}
