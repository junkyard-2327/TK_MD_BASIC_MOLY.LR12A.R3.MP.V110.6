...
static bool check_certificate(private_credential_manager_t *this,
				...
				int pathlen, bool trusted, auth_cfg_t *auth, int *status_code)
{
	cert_validator_t *validator;
	enumerator_t *enumerator;
	...
	{
		if (status_code && *status_code == PDN_STATUS_OK)
		{
			*status_code = PDN_STATUS_INVALID_CERTIFICATE;
		}
		...
		trusted, auth))
	{
		if (status_code && *status_code == PDN_STATUS_OK)
		{
			*status_code = PDN_STATUS_INVALID_CERTIFICATE_EPDG;
		}
...
static bool verify_trust_chain(private_credential_manager_t *this,
							   ...
							   bool trusted, bool online, bool certcheck,
							   int *status_code)
{
	...
	for (pathlen = 0; pathlen <= MAX_TRUST_PATH_LEN; pathlen++)
	{
		...
		if (!check_certificate(this, current, issuer, online,
							   pathlen, trusted, auth, status_code))
		{
			trusted = FALSE;
			issuer->destroy(issuer);
			break;
...
static bool trusted_enumerate_orig(trusted_enumerator_t *this,
	certificate_t **cert, auth_cfg_t **auth, int *status_code)
{
		...
		if (this->pretrusted)
		{
			...
			if (issued_by(this->this, this->pretrusted, this->pretrusted, NULL) ||
				verify_trust_chain(this->this, this->pretrusted, this->auth,
								   TRUE, this->online, this->certcheck, status_code))
			{
	...
	while (this->candidates->enumerate(this->candidates, &current))
	{
		...
		if (verify_trust_chain(this->this, current, this->auth, FALSE,
							   this->online, this->certcheck, status_code))
		{
			*cert = current;
			if (auth)
			{
...
{
	return trusted_enumerate_orig(this, cert, auth, NULL);
}

METHOD(enumerator_t, trusted_enumerate2, bool,
	trusted_enumerator_t *this, certificate_t **cert, auth_cfg_t **auth, int *status_code)
{
	return trusted_enumerate_orig(this, cert, auth, status_code);
}
...
METHOD(credential_manager_t, create_trusted_enumerator2, enumerator_t*,
	private_credential_manager_t *this, key_type_t type,
	identification_t *id, bool online, bool certcheck)
{
	trusted_enumerator_t *enumerator;

	INIT(enumerator,
		.public = {
			.enumerate = (void*)_trusted_enumerate2,
			.destroy = _trusted_destroy,
		},
		.this = this,
		.type = type,
...
METHOD(enumerator_t, public_enumerate2, bool,
	public_enumerator_t *this, public_key_t **key, auth_cfg_t **auth, int *status_code)
{
	certificate_t *cert;

	while (this->inner->enumerate(this->inner, &cert, auth, status_code))
	{
		DESTROY_IF(this->current);
		this->current = cert->get_public_key(cert);
		if (this->current)
...
METHOD(credential_manager_t, create_public_enumerator2, enumerator_t*,
	private_credential_manager_t *this, key_type_t type, identification_t *id,
	auth_cfg_t *auth, bool online, bool certcheck)
{
	public_enumerator_t *enumerator;

	INIT(enumerator,
		.public = {
			.enumerate = (void*)_public_enumerate2,
			.destroy = _public_destroy,
		},
		.inner = create_trusted_enumerator2(this, type, id,
								online, certcheck),
		.this = this,
	);
	if (auth)
...
credential_manager_t *credential_manager_create()
{
	...
	INIT(this,
		.public = {
			...
			.create_public_enumerator2 = _create_public_enumerator2,
