	...
	while (TRUE)
	{
		...
		switch (sig)
		{
			...
			case SIGINT:
			{
				...
				lib->scheduler->release_alarm_timer_wake_lock(
						lib->scheduler, JOB_PRIO_HIGH_WAKE_LOCK, TRUE);
			...
			case SIGABRT:
			{
				DBG1(DBG_DMN, "signal of type SIGABRT received. Shutting down");
				lib->scheduler->release_alarm_timer_wake_lock(
						lib->scheduler, JOB_PRIO_HIGH_WAKE_LOCK, TRUE);
				return;
			}
			case SIGSEGV:
			{
				DBG1(DBG_DMN, "signal of type SIGSEGV received. Shutting down");
				lib->scheduler->release_alarm_timer_wake_lock(
						lib->scheduler, JOB_PRIO_HIGH_WAKE_LOCK, TRUE);
				return;
			}
