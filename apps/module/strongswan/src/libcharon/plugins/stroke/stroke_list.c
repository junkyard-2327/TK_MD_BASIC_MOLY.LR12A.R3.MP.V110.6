	...
	private_stroke_list_t *this, stroke_msg_t *msg, FILE *out)
{
	...
	{
		if (streq(msg->force_dpd.name, ike_sa->get_name(ike_sa)))
		{
	...
	while (enumerator->enumerate(enumerator, &ike_unique_id))
	{
		ike_sa = charon->ike_sa_manager->checkout_by_id(charon->ike_sa_manager,
														ike_unique_id, FALSE);
		if (ike_sa) {
			ike_sa->send_force_dpd(ike_sa, (u_int32_t)msg->force_dpd.retran_to,
								(u_int16_t)msg->force_dpd.retran_tries,
								(u_int16_t)msg->force_dpd.retran_base);
	...
	{
		DBG1(DBG_CFG, "no IKE_SA named '%s' found", msg->force_dpd.name);
	}
