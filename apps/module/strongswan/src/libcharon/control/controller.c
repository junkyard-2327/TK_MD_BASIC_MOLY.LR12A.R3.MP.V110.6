	...
	interface_job_t *job)
{
	...
	u_int32_t action = listener->action;
	ike_sa_t *ike_sa;
	...
	if (ike_sa->delete(ike_sa, action) != DESTROY_ME)
	{	/* delete failed */
		listener->status = FAILED;
		charon->ike_sa_manager->checkin(charon->ike_sa_manager, ike_sa);
	}
...
METHOD(controller_t, terminate_ike, status_t,
	...
	controller_cb_t callback, void *param, u_int action, u_int timeout)
{
	interface_job_t *job;
	status_t status;
