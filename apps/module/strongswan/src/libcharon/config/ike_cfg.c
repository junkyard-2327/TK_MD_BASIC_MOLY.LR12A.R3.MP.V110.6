...
struct private_ike_cfg_t {
	...
	/**
	 * our source natt port
	 */
	u_int16_t my_natt_port;

	/**
	 * destination natt port
	 */
	u_int16_t other_natt_port;
...
METHOD(ike_cfg_t, get_my_natt_port, u_int16_t,
	private_ike_cfg_t *this)
{
	return this->my_natt_port;
}

METHOD(ike_cfg_t, get_other_natt_port, u_int16_t,
	private_ike_cfg_t *this)
{
	return this->other_natt_port;
}
	...
	while (stored_enum->enumerate(stored_enum, (void**)&stored))
	{
		...
		while (supplied_enum->enumerate(supplied_enum, (void**)&supplied))
		{
			...
			if (selected)
			{
				...
				DBG_WITH_ANDROID_PROPERTY(ENCR_KEY_DISPLAY, DBG_KEY, "selected proposal: %P", selected);
				return selected;
			}
		}
	}
...
METHOD(ike_cfg_t, set_wod_options, void,
	private_ike_cfg_t *this, u_int16_t my_natt_port, u_int16_t other_natt_port)
{
	this->my_natt_port = my_natt_port;
	this->other_natt_port = other_natt_port;
}
						  ...
						  fragmentation_t fragmentation, u_int8_t dscp)
{
	...
	INIT(this,
		.public = {
			...
			.get_my_natt_port = _get_my_natt_port,
			.get_other_natt_port = _get_other_natt_port,
			.get_dscp = _get_dscp,
