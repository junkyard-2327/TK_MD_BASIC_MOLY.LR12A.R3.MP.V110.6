 ...
 */
static void send_ike_delete_informational(private_ike_auth_t *this,
										   message_t *reply)
{
	message_t *message;
	packet_t *packet;
...
//						return FAILED;
	...
	if(is_error)
	{
		...
		if(peer_cfg)
		{
			if(peer_cfg->send_del_ike_auth_ntfy_err(peer_cfg))
			{
				int error_code = this->ike_sa->get_sub_error_code(this->ike_sa);
				if (error_code != INVALID_SYNTAX && error_code != UNSUPPORTED_CRITICAL_PAYLOAD
					&& error_code != AUTHENTICATION_FAILED)
				{
					DBG1(DBG_IKE, "Send IKE_DELETE after special notify failed");
