...
static configuration_attribute_t *build_liveness_check(u_int32_t timeout_period)
{
	...
	else
	{
		...
		timeout_period = htonl(timeout_period);
		memcpy(chunk.ptr, (void *)&timeout_period, chunk.len);
	}

	DBG1(DBG_IKE, "building %N attribute, timeout_period: %u", configuration_attribute_type_names, type, timeout_period);
	return configuration_attribute_create_chunk(CONFIGURATION_ATTRIBUTE, type, chunk);
}
...
static configuration_attribute_t *build_imei(peer_cfg_t *config)
{
	...
	int format = config->get_imei_format(config);
	u_char *imei = config->get_imei(config);
	chunk_t chunk;

	if(format == IMEI_STRING)
	{
		chunk = chunk_create(imei, strlen(imei));
	}
	else
	{
		chunk = chunk_create(config->get_imei_bcd_encoded(config), config->get_imei_bcd_encoded_size(config));
	}
	DBG1(DBG_IKE, "building %N attribute", configuration_attribute_type_names, type);
	return configuration_attribute_create_chunk(CONFIGURATION_ATTRIBUTE, type, chunk);
}
							  ...
							  configuration_attribute_t *ca)
{
	...
	switch (ca->get_type(ca))
	{
		...
		/* If liveness check feature is enabled, it will be set to the DPD timeout value*/
		case TIMEOUT_PERIOD_FOR_LIVENESS_CHECK:
			if(config->get_liveness_check(config))
			{
				chunk_t value;
				u_int32_t liveness_timeout = 0;
				value = ca->get_chunk(ca);
				if(!value.len)
				{
					DBG1(DBG_IKE, "No value in TIMEOUT_PERIOD_FOR_LIVENESS_CHECK attribute");
				}
				else if(value.len == 4)
				{
					memcpy((void *)&liveness_timeout, value.ptr, value.len);
					liveness_timeout = ntohl(liveness_timeout);
					config->set_liveness_check_timeout(config, liveness_timeout);
					config->set_dpd(config, liveness_timeout);
					DBG1(DBG_IKE, "Set DPD timeout: %u", liveness_timeout);
				}
				else
				{
					DBG1(DBG_IKE, "TIMEOUT_PERIOD_FOR_LIVENESS_CHECK invalid len : %u", value.len);
				}
			}
			break;
	...
	private_ike_config_t *this, message_t *message)
{
	...
	{	/* in first IKE_AUTH only */
		...
		if (config->get_cust_cp_imei(config) > 0 && config->get_imei_bcd_encoded(config))
		{
			if(!cp)
			{
				cp = cp_payload_create_type(CONFIGURATION, CFG_REQUEST);
