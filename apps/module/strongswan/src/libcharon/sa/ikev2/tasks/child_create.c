	...
	private_child_create_t *this, message_t *message)
{
	...
	if(this->rekey && this->tsi_rekey && this->tsr_rekey)
	{
		DBG0(DBG_IKE, "build child_sa rekey request with init tsi/tsr");
	...
	if(!this->rekey || !this->tsi_rekey || !this->tsr_rekey)
	{
		DBG1(DBG_IKE, "it is not rekey, destroy tsi & tsr");
		this->tsi->destroy_offset(this->tsi, offsetof(traffic_selector_t, destroy));
		this->tsr->destroy_offset(this->tsr, offsetof(traffic_selector_t, destroy));
	}
	...
	private_child_create_t *this, message_t *message)
{
	...
	/*check if rekey request, if it is a rekey request, it need contain the init TS*/
	if (this->rekey) {
		/* store the tsi & tsr for next rekey used */
		if (this->child_sa)
		{
			this->child_sa->set_rekey_other_ts(this->child_sa, this->tsi->clone_offset(this->tsi,
																				offsetof(traffic_selector_t, clone)));
			this->child_sa->set_rekey_my_ts(this->child_sa, this->tsr->clone_offset(this->tsr,
																				offsetof(traffic_selector_t, clone)));
			DBG1(DBG_IKE, "store tsi for rekey other_ts and tsr for rekey my_ts");
		}
	}
	...
	private_child_create_t *this, message_t *message)
{
	...
	if (this->child_sa)
	{
		this->child_sa->set_rekey_my_ts(this->child_sa, this->tsi->clone_offset(this->tsi,
																				offsetof(traffic_selector_t, clone)));
		this->child_sa->set_rekey_other_ts(this->child_sa, this->tsr->clone_offset(this->tsr,
																				offsetof(traffic_selector_t, clone)));
		DBG1(DBG_IKE, "store tsi for rekey my_ts and tsr for rekey other_ts");
	}
