	...
	private_ike_rekey_t *this, message_t *message)
{
	...
	this->ike_delete = ike_delete_create(this->ike_sa, FALSE, 0);
	...
	private_ike_rekey_t *this, message_t *message)
{
		...
		this->collision->get_type(this->collision) == TASK_IKE_REKEY)
	{
		...
		if (other->ike_init)
		{
			...
			else
			{
				...
				if (this->new_sa->delete(this->new_sa, 0) == DESTROY_ME)
				{
	...
	/* rekeying successful, delete the IKE_SA using a subtask */
	this->ike_delete = ike_delete_create(this->ike_sa, TRUE, 0);
