	...
	{	
		if (!this->old_sa && this->initiator && peer_cfg->get_redirect_enable(peer_cfg))
		{
			...
			if (from)
			{
				...
				DBG1(DBG_IKE, "add REDIRECTED_FROM notify");
				chunk_free(&data);
			...
			else
			{
				message->add_notify(message, FALSE, REDIRECT_SUPPORTED,
								chunk_empty);
				DBG1(DBG_IKE, "add REDIRECT_SUPPORTED notify");
			}
		}
	}
}
		...
		if (payload->get_type(payload) == NOTIFY)
		{
			...
			switch (type)
			{
				...
				case REDIRECT:
				{
					...
					data = notify->get_notification_data(notify);
					gateway = redirect_data_parse(data, &nonce);
					this->ike_sa->set_redirect_state(this->ike_sa, REDIRECT_IKE_SA_ESTABLISHING_START);
