										 ...
										 eap_payload_t *in)
{
	...
	if (this->method == NULL)
	{
		...
		DBG1(DBG_IKE, "EAP ID: '%Y'", SENSITIVE_LOG_ID(id));
		this->eap_identity = id->clone(id);
						...
						chunk_t nonce, chunk_t init)
{
	...
	DBG1(DBG_IKE, "authentication of '%Y' with %N successful",
		 SENSITIVE_LOG_ID(other_id), auth_class_names, AUTH_CLASS_EAP);
					   ...
					   chunk_t nonce, chunk_t init)
{
	...
	DBG1(DBG_IKE, "authentication of '%Y' (myself) with %N",
		 SENSITIVE_LOG_ID(my_id), auth_class_names, AUTH_CLASS_EAP);
