...
METHOD(ike_sa_t, delete_, status_t,
	private_ike_sa_t *this, int action)
{
	switch (this->state)
	{
		case IKE_REKEYING:
		...
		case IKE_ESTABLISHED:
			...
			this->task_manager->queue_ike_delete(this->task_manager, action);
			if(action == 1)
			{
				/* Terminate SA as immediately as possible */
				if (this->task_manager->initiating_type(this->task_manager) != EXCHANGE_TYPE_UNDEFINED)
				{
					/* Terminate SA just now, can't send DELETE to NW, 
					 * because there was previous exchange not completed,
					 * so mid for this DELETE req can't be assigned (will be invalid) */
					DBG1(DBG_IKE, "Immediate terminate SA, without sending DELETE as some exchange on air");
					if (get_oos(this))
					{
						set_oos(this, OOS_END);
						DBG1(DBG_IKE, "============================= oos disabled.");
				...
				{
					DBG1(DBG_IKE, "Immediate terminate SA, after sending DELETE");
					this->task_manager->flush_queue(this->task_manager, TASK_QUEUE_ACTIVE);
					return this->task_manager->initiate(this->task_manager);
				}
			} 
			else
