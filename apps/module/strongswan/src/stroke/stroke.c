...
static int terminate_connection(char *name, char *action)
{
	stroke_msg_t msg;

	msg.type = STR_TERMINATE;
	msg.length = offsetof(stroke_msg_t, buffer);
	msg.terminate.name = push_string(&msg, name);
	msg.terminate.action = push_string(&msg, action);
	return send_stroke_msg(&msg);
}
...
int main(int argc, char *argv[])
{
	...
	switch (token->kw)
	{
		...
		case STROKE_DOWN:
			...
			res = terminate_connection(argv[2], argc > 3 ? argv[3] : NULL);
			break;
		case STROKE_DPD:
			if (argc < 6)
			{
